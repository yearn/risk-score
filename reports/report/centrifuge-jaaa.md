...
- **Cross-chain messaging — 2/2 threshold, LayerZero V2 paired with Axelar or Chainlink CCIP.** The [`MultiAdapter`](https://etherscan.io/address/0x35C837F0A54B715a23D193E1476BFC9BC30073BE) (live: `Gateway 0x19a5…3172 .adapter()`) was re-read per destination for the **JAAA pool specifically** — `adapters(centrifugeId, 281474976710663, i)`, `quorum`, `threshold` — at block 25796082. Adapter identity was taken from verified source, not from the address alone: [`0xD517…0295`](https://etherscan.io/address/0xD517BC7ba17271a8D87BE7355B2523bF5c750295) is `LayerZeroAdapter`, [`0x34e9…6484`](https://etherscan.io/address/0x34e904237341C3de02D4447C3fF0ca8880ca6484) is **`AxelarAdapter`** (`axelarGateway()` / `axelarGasService()`), and [`0x39CF…D955`](https://etherscan.io/address/0x39CF679Eb0Ac9075CFb5f94930A367Ba1557D955) is `ChainlinkAdapter` (`ccipRouter()` / `ccipReceive()`). A prior version of this report misattributed `0x34e9…6484` to Chainlink CCIP.
  ...
  **The 2-of-2 framing does not hold universally, and X Layer's zero supply does not bound the exposure.** `MultiAdapter.handle()` contains an explicit fast path — `if (adapter.quorum == 1) { gateway.handle(centrifugeId, payload); return; }` — so on X Layer a single LayerZero delivery executes a JAAA-scoped message with no second attestation. The route is wired (`LayerZeroAdapter.isWired(13) = true`) and the share token is deployed ([`0xAD48…F797`](https://www.oklink.com/xlayer/address/0xAD48F183E586e92A591A610397ebf534609DF797)).
  ...
  Traced end to end in verified source, an authenticated payload from that route reaches an **unbacked mint on Ethereum**:

  1. `MessageLib.messagePoolId(message)` returns `message.toUint64(1)` for every message type at or after `NotifyPool`. `ExecuteTransferShares` is one of them, so the **poolId that selects the adapter set is a field inside the message** — a forged payload naming JAAA's poolId is routed under JAAA's X Layer set (LayerZero only, quorum 1).
  2. `MessageProcessor.handle` dispatches it: `spoke.executeTransferShares(PoolId.wrap(m.poolId), ShareClassId.wrap(m.scId), m.receiver, m.amount)`. MessageProcessor [`0x97cc…e30A`](https://etherscan.io/address/0x97cc7e9Dafdd725Cc23B25eeBC93c4384B4Fe30A) is a Spoke ward (`wards = 1`), so the `auth` modifier passes.
  3. `Spoke.executeTransferShares` is three lines: `shareToken_.mint(address(this), amount); shareToken_.transfer(receiver.toAddress(), amount);`. **There is no check against remote supply, no burn proof, and no amount cap.** `amount` is whatever the message says.

  So a compromise of the LayerZero path for centrifugeId 13 forges an arbitrary Ethereum JAAA mint. X Layer's balance of zero constrains only *honest* transfers out of X Layer; it is not a constraint on a forged message. What does constrain the attack:

  - **Receiver must be allowlisted.** The `transfer` leg hits `FullRestrictions.checkERC20Transfer` via `isCrosschainTransferExecution(from = Spoke, to = receiver) → isTargetMember(to)`, so the minted shares must land on a KYC-allowlisted (or Root-endorsed) address or the whole message reverts. `crosschainSource` is confirmed onchain as the Spoke, and `hookDataOf(Spoke)` carries a far-future membership expiry, so the intermediate `mint` to the Spoke itself passes. This bounds *who* can receive, not *how much* is minted.
  - **The LayerZero leg is not a single key.** That route still enforces 15 confirmations and the same 4-of-5 DVN quorum as every other route. The exposure is "defeat LayerZero's DVN set for this route", not "forge one signature" — but it is exactly the failure the 2-of-2 quorum is designed to survive everywhere else, and here there is no second system to survive it.
...
- **One destination is a single-adapter route that can mint.** The JAAA pool's MultiAdapter set for X Layer (centrifugeId 13) is LayerZero V2 alone at `quorum() = threshold() = 1`. Every other JAAA destination is 2-of-2. See Critical Risks — this is not bounded by X Layer's zero supply.

### Critical Risks `[If Any]`

- **A single-adapter cross-chain route can mint unbacked JAAA on Ethereum.** The JAAA pool's MultiAdapter set for X Layer (centrifugeId 13) is LayerZero V2 alone at `quorum() = threshold() = 1`. Because `MessageLib.messagePoolId` reads the poolId out of the message body, a forged `ExecuteTransferShares` naming JAAA's poolId is routed under that single-adapter set; `MessageProcessor` then calls `Spoke.executeTransferShares`, which mints the message's `amount` with no remote-supply check, no burn proof and no cap. X Layer's current supply of 0 does not bound this — it only bounds honest transfers. Two real constraints apply: the receiver must be a KYC-allowlisted or Root-endorsed address (`FullRestrictions` hook), and the LayerZero route still enforces its 4-of-5 DVN quorum. Neither restores the independent second attestation that every other JAAA destination has. Remediation is a one-call fix — `setAdapters(13, poolId, [layerZero, chainlink], 2, 2)` — so the gap is configuration, not architecture.
...
- **Cross-chain:** **2-of-2 MultiAdapter quorum on six of seven configured destinations** — LayerZero V2 + **Axelar** on the four highest-supply remote chains (Base, Arbitrum, Avalanche, BNB); LayerZero V2 + Chainlink CCIP on Monad and Pharos, of which **Monad carries live supply**, so both Axelar and CCIP are live dependencies. The LayerZero leg's ten configured inbound routes each use 15 confirmations and a 4-of-5 effective DVN quorum (Deutsche Telekom + Canary required, plus 2-of-3 P2P/Nansen/Nethermind), materially stronger than the prior single-Wormhole setup.
- **Cross-chain, single-adapter exception:** the JAAA pool's X Layer route is LayerZero alone at `quorum() = 1`, and that route can mint JAAA on Ethereum without a second attestation (see Critical Risks).
...
→ **Subcategory C: 2.5** — 2-of-2 cross-chain quorums with top-tier providers on six of seven configured destinations, plus multi-stablecoin settlement, materially reduce onchain-dependency concentration. Held above 2.0 by the Trident Trust dual-role concentration (see Provability) and the still-newer Anemoy partnership (~12 months onchain).

  **The X Layer single-adapter route was considered here and does not move this score.** The reasoning, stated explicitly so a reader can disagree with it: the route carries no supply and no economic activity; every destination that does carry supply is 2-of-2; the LayerZero leg guarding it is the same 4-of-5 DVN quorum used everywhere else, so this is a missing *second* system rather than an unguarded one; and remediation is a single `setAdapters(13, poolId, [layerZero, chainlink], 2, 2)` call available to existing governance without a migration. A subcategory score is a standing assessment of dependency structure, and the structure Centrifuge operates is 2-of-2 — this is a configuration gap in it. The counter-argument is real and recorded in Critical Risks: the exposure is an unbounded mint, it is live from the moment a route is wired rather than from the moment supply appears, and "no supply yet" is a weak guarantee. If that route gains supply, or a second quorum-1 destination appears, this subcategory should move.
...
**Final Score: 2.6/5.0** — unchanged. The August 20, 2026 cross-chain re-verification (adapter set corrected from Chainlink CCIP to Axelar; X Layer single-adapter mint path identified) was assessed against Subcategory C and did not move it — see the reasoning recorded there. Removing a parallel manager is directionally positive, while current TVL/AUM and broader holdings also strengthen the profile. Those improvements do not yet overcome the score-driving issue: the sole, offchain-attested Fordefi MPC can still set an unbounded canonical NAV and exercise untimelocked pool powers. Morpho's Chronicle feed is useful attestation protection for that market, but it does not make direct JAAA exposure low risk or enforce a dynamic bound on Centrifuge's price.
...
- **Cross-chain adapter set changes:** Reassess if the active MultiAdapter quorum changes (e.g., an Axelar, Chainlink CCIP or LayerZero V2 adapter is added/removed for the JAAA pool), if `quorum()` is lowered on any destination, or if a `SetAdapters` event fires for poolId `281474976710663`. **Reassess immediately downward-favourable if X Layer (centrifugeId 13) gains a second adapter**, and treat any *new* destination wired at quorum 1 as a fresh instance of the Critical Risk above — the exposure is present from the moment the route is wired, not from the moment supply appears there.
