...
- Saturn's own [key-addresses page](https://saturncredit.gitbook.io/saturn-docs/operations-and-governance/key-addresses) now also lists USDat/sUSDat deployments on **BNB Chain** and **Monad** — a new cross-chain surface not present in the June 17, 2026 assessment, and **not small**: BNB Chain USDat total supply = **1,989,731.37** (~$2.0M) and **Monad USDat total supply = 27,598,577.36** (~$27.6M, same address `0x0Bb150DFa86EA5d7742F07FEfCD8E8edA81D64eF` on both chains), both verified on-chain September 9, 2026. **Combined non-Ethereum USDat supply (~$29.6M) is roughly 46% of Ethereum mainnet's own supply (64.1M).** The bridge is **Chainlink CCIP**, mixed lock/mint: Ethereum locks real USDat in a `LockReleaseTokenPool` (31,965,633.03 USDat escrowed — about half of Ethereum's own `totalSupply()`), while BNB Chain and Monad each run a `BurnWithFromMintTokenPool` that can mint canonical USDat independent of that lockbox's real-time balance. Full detail in External Dependencies and [`src/data/bridges.json`](../../src/data/bridges.json).
...
**Five audit reports are published** (Saturn [Transparency & Audits](https://saturncredit.gitbook.io/saturn-docs/operations-and-governance/transparency-and-audits): "Certora - Formal Verification.pdf", "Certora - Audit #3.pdf", "Certora - Audit #2.pdf", "Three Sigma - Audit #1.pdf", "Certora - Saturn-sUSDat v2.pdf"), all reviewed for this assessment:
...
| 5 | **Certora** ("Saturn-sUSDat v2") | **TODO** (filename only; PDF not opened this session) | **sUSDat specifically** (v2 — a follow-up/updated review, by name), not the PYUSDx-backed USDat architecture | **TODO** | **TODO — plausible but unconfirmed connection to the Innora SAT-001/002/003/004 disclosure below (both target the sUSDat vault); could be Certora's follow-up covering fixes to those findings, or an unrelated later review. Needs the PDF opened to confirm.** |

So coverage is strong on paper — two reputable firms, five reports including formal verification and a second-pass sUSDat review. Importantly, Certora Audit #3 is specifically the **Saturn Dollar M0 Extensions** review and covers the M0 `JMIExtension` architecture used by deployed USDat (`0x2323…aa71`, impl `0x17ca…e52e`), including manual review and Certora Prover verification of the contracts in scope. The fifth report's title indicates it targets **sUSDat**, not USDat — it does not, on the evidence available, extend audit coverage to USDat's PYUSDx-backed architecture (see gap below).
...
- **Independent security disclosure found this session, not in any of the four originally-reviewed Saturn-published audits or the June 17, 2026 report:** [Innora Security Research](https://gist.github.com/sgInnora/b70ad98327649ed4ab976a122f45e485) published **SAT-001 (Critical)**, **SAT-002 (High)**, **SAT-003 (Medium)**, and **SAT-004 (Informational)** findings against the **sUSDat vault** on **April 14, 2026** (researcher: Feng Ning) — SAT-001 describes an arithmetic underflow in `convertFromStrc()`/`burnQueuedShares()` capable of freezing withdrawals for up to 30 days; SAT-002 describes the PROCESSOR role extracting up to 33.33% per conversion cycle via independently-applied tolerances in `_validateConversion()`; SAT-003 describes front-running risk in `redistributeLockedAmount()`. The researcher reported disclosing immediately rather than via standard 90-day coordinated disclosure, citing "structural indicators suggesting intentional design," and that the contract was **unpatched, unpaused, unaudited** as of the disclosure date. **This session could not independently verify current patch/exploit status, whether Saturn responded, or whether findings are disputed** — `sUSDat.paused()` reads **false** as of September 8, 2026 (no active pause), but that alone doesn't confirm a fix. The **fifth, sUSDat-titled Certora audit** (see table above) is a plausible but **unconfirmed** candidate for Saturn's response to this disclosure — same target contract, "v2" naming consistent with a follow-up review — but this was not verified against the PDF this session. This affects **sUSDat** (context layer), not USDat's own backing, but is directly relevant to the "sUSDat context risk" already carried in this report's Key Risks, and to Saturn's operational/security-responsiveness track record. **TODO: high-priority follow-up** — open the fifth audit PDF and determine current status of SAT-001/002/003/004.

### Bug Bounty `[If Applicable]`

- No bug bounty program — confirmed absent from the docs as of June 17, 2026 (via the GitBook docs Q&A endpoint) and not found on Immunefi; independently corroborated by the Innora disclosure above ("no publicly listed security contact").
- **Safe Harbor (SEAL) adoption: confirmed absent** — Saturn does not have a Safe Harbor agreement in place.
...
- Concentration risk from large depositors / holder distribution: reviewed — holders are predominantly other DeFi protocols (pools, vaults) rather than concentrated retail/individual wallets, which is a reasonable distribution for a collateral asset; exact holder-by-holder percentages were not independently pulled via Etherscan Pro/Dune this session, so precise concentration figures remain **TODO**. sUSDat NAV (totalAssets ≈ $74.41M, September 8, 2026) exceeds USDat's own total supply (64.10M), confirming sUSDat's `totalAssets()` is an off-chain NAV accounting figure (including STRC exposure) rather than a literal on-chain USDat balance.
...
- **New, materially large cross-chain surface (not present June 17, 2026), now mapped via [Chainlink CCIP](https://chain.link/cross-chain):** Saturn's key-addresses page lists USDat/sUSDat deployments on **BNB Chain** and **Monad**. Verified on-chain September 9, 2026, using the standard CCIP `TokenAdminRegistry` lookup (`getPool`, `typeAndVersion`): this is a **mixed lock/mint bridge**. Ethereum-side USDat routes through a `LockReleaseTokenPool` ([`0xA0dE671eb84dE9dEe1d311d996d25c41F63a0D86`](https://etherscan.io/address/0xA0dE671eb84dE9dEe1d311d996d25c41F63a0D86)) holding **31,965,633.03 USDat** — real, escrowed collateral that is part of Ethereum's own 64.1M `totalSupply()`, i.e. roughly **half of Ethereum-resident USDat is locked backing for the other chains, not freely circulating on Ethereum itself.** On BNB Chain and Monad, USDat is a separate `SaturnOFT`-based token contract (same bytecode both chains, verified source, author-tagged "LayerZero Labs" in its NatSpec despite the live transport being CCIP) whose `MINTER_ROLE`/`BURNER_ROLE` is held by a `BurnWithFromMintTokenPool` — BNB Chain [`0x904939A965eDc2Efbd44Fd4db223f07680510C61`](https://bscscan.com/address/0x904939A965eDc2Efbd44Fd4db223f07680510C61) (~$2.0M circulating) and Monad [`0x8ab40e2dabCd24dCFf0A06A68eB59990405270F0`](https://explorer.monad.xyz/address/0x8ab40e2dabCd24dCFf0A06A68eB59990405270F0) (~$27.6M circulating, ~46% of Ethereum's supply). Because the mint-side pools can create canonical supply on their own chain independent of the Ethereum lockbox's real-time balance, this is the higher-blast-radius route (classified `mint` in [`src/data/bridges.json`](../../src/data/bridges.json) per the bridge skill's prioritization rule). **Governance is split and weaker than Ethereum's own:** the Ethereum lock pool is owned by an `RBACTimelock` (`0x44835bBBA9D40DEDa9b64858095EcFB2693c9449`, 3-hour delay); the BNB/Monad side is administered by a `SaturnTimelock` (`0xF0237a1cb794eA830129966049eF99243082FF0C`, identical address both chains, **2-day delay** — shorter than Ethereum's own 5-day Admin Timelock).
...
- **Team:** Backed by reputable investors (**YZi Labs**, **Sora Ventures**, **Spartan**). Founder/team identities and track record: **not determinable with available tools** — checked this session; web search returned only unrelated companies also named "Saturn," and Saturn's own Terms of Service (see Legal structure below) name the operating entity but not individual team members.
...
- **Legal structure / jurisdiction:** Per the June 17, 2026 DD docs review, a **Cayman foundation** owns a **BVI token issuer ("Saturn Capital")** that receives user stablecoins and issues USDat; when USDat is staked, Saturn Capital invests via a **regulated BVI fund ("Saturn Fund")** that holds the STRC; the smart-contract layer is launched under **Panama** jurisdiction. Off-chain service providers: **Galaxy** (execution broker / on-off-ramp), **Clear Street** (STRC custody, Galaxy's partner), **Securitize** (fund administrator/transfer agent), **Fireblocks** (key management). **Directly sourced this session from Saturn's Terms of Service** (`https://saturn.credit`): the contracting entity is stated as **"Saturn Global Capital Investments Ltd., a British Virgin Islands limited company"** governing use of the Interface and Services. This is consistent with a BVI-incorporated operating entity but uses a **different name** ("Saturn Global Capital Investments Ltd.") than the DD-doc-sourced "Saturn Capital" — plausibly the same entity under a fuller legal name, a rename, or a distinct affiliate; **not reconciled this session**. Note: per the DD docs, "ownership claims cannot be enforced in court for the capital backing the protocol" — relevant mainly to the **sUSDat/STRC** layer (USDat's backing, now PYUSDx, is held on-chain in the token contract).
...
| CCIP lockbox coverage | `USDat.balanceOf(0xA0dE…0D86)` (Ethereum `LockReleaseTokenPool`) vs. combined BNB Chain + Monad USDat `totalSupply()` | Alert if locked balance drops below combined remote circulating supply |
| Cross-chain supply | BNB Chain USDat (`0x0Bb1…64eF`) and Monad USDat (same address) `totalSupply()` | Track growth of the mint-side (higher blast-radius) route |
| CCIP pool/admin changes | `TokenAdminRegistry.getPool(USDat)` on each chain; `RBACTimelock`/`SaturnTimelock` role and ownership changes | Alert on any pool or admin change — remote-chain timelock (2d) is shorter than Ethereum's own (5d) |
...
- **Unaudited architecture change:** none of the five published audits (including the April 30, 2026 Certora Audit #3, and a fifth, sUSDat-titled Certora report that appeared after the June 17, 2026 assessment) cover the `replaceAsset` mechanism or the PYUSDx-backed design now live in production — the fifth audit's title indicates it targets sUSDat, not USDat.
...
- **Unresolved third-party security disclosure on sUSDat (context, not scored into USDat):** Innora Security Research published a Critical-severity finding (potential 30-day withdrawal freeze) and a High-severity finding (PROCESSOR-role over-extraction) against the sUSDat vault on April 14, 2026 — before, and unaddressed by, both Saturn's own audits and the June 17, 2026 report. Current patch/exploit status could not be confirmed this session (see Audits and Due Diligence Disclosures). This targets sUSDat's STRC-conversion mechanics specifically; absent a demonstrated mechanical link to USDat's own mint/redeem/backing path, it is **not** factored into this report's Risk Score Assessment below, but is worth tracking given Saturn's shared governance and reputational surface.
- **New cross-chain surface, now mapped:** USDat is deployed on BNB Chain (~$2.0M) and **Monad (~$27.6M — ~46% of Ethereum's own supply)** via **Chainlink CCIP**. Ethereum locks real USDat (31.97M, ~half of Ethereum's own supply) to back the remote chains, but the remote-chain `BurnWithFromMintTokenPool`s can mint canonical USDat independent of that lockbox, governed by a separate, **shorter (2-day)** timelock than Ethereum's own 5-day Admin Timelock.
...
- **Audits:** Five reports are now on record (1× Three Sigma + 3× Certora incl. formal verification + 1× Certora sUSDat follow-up), zero unresolved Highs/Criticals in the public summaries for the original four. However, the most recent audit covering USDat itself (Certora Audit #3, April 30, 2026) **predates and does not cover** the August 19, 2026 PYUSDx migration or the `replaceAsset` mechanism, and the fifth report is sUSDat-scoped by title — **USDat's own, currently-live backing architecture is effectively unaudited.** This is closer to "coverage exists but is dated / doesn't cover the current design" than "actively covered" → **3.0** (up from 2.5 on June 17, 2026). (The separate Innora sUSDat disclosure — see Audits and Due Diligence Disclosures — is noted as context but is **not** used to justify this score: it targets sUSDat's STRC-conversion/vault mechanics specifically, with no demonstrated mechanical link to USDat's own mint/redeem/backing path that this report scores, and its current status is unverified. It is scored, if at all, only insofar as it reflects on Saturn's general audit/disclosure posture via the Operational Risk category below — not folded into this number.)
...
- **Audit-based:** Reassess once the PYUSDx-backed architecture and `replaceAsset` mechanism receive dedicated third-party audit coverage, or once the fifth ("Saturn-sUSDat v2") audit's content is confirmed and found to bear on USDat.
...
**Resolved this session** (previously open, now confirmed — see inline citations above): the `replaceAsset` mechanism, caller, and authority (§Funds Management); the `isAllowedAsset(PYUSDx)`/`assetCap(PYUSDx)` behavior (by-design exclusion, not an anomaly); the USDat-level redemption first leg and fee structure (§Accessibility); the prior $10M USDC-cap question (no direct analog in the new design); the BNB Chain and Monad USDat supply figures and proxy verification status; on-chain slippage quotes for $1-5M USDat exits (§Liquidity Risk); the **BNB Chain / Monad bridge mechanism** (§External Dependencies — Chainlink CCIP, mixed lock/mint model, pools and admins identified, recorded in `src/data/bridges.json`); the **fifth audit's title** (Certora, "Saturn-sUSDat v2" — sUSDat-scoped, does not extend to USDat's PYUSDx architecture; date/findings still TODO); **Safe Harbor (SEAL)** status (confirmed absent); **holder distribution** (qualitatively reviewed — predominantly DeFi protocols, not concentrated retail wallets); **legal entity name** (Saturn's ToS names "Saturn Global Capital Investments Ltd.," a BVI company — see Operational Risk for the naming discrepancy against the DD-doc-sourced "Saturn Capital").

**Removed:** the shared-`RBACTimelock`-address item (previously flagged as unreconciled against Maple syrupUSDC's CCIP pool) was reviewed and dropped as not materially relevant to this report's scope.

**Still open:**

1. Open the **fifth audit PDF** ("Certora - Saturn-sUSDat v2.pdf") to get its date and findings, and confirm/deny whether it addresses the Innora SAT-001/002/003/004 disclosure below — blocked by JavaScript-rendered content this session (a Playwright browser tool was available but its permission grant did not go through despite repeated attempts).
2. Determine current status of the **Innora Security Research SAT-001/002/003/004 disclosure** against sUSDat (published April 14, 2026): patched, disputed, or still live — see item 1 above, the most likely path to resolving this. (Scored, if at all, under Saturn's general disclosure posture — not folded into USDat's own Audits score; see Risk Score Assessment.)
...
4. Assess **M0's, MoonPay's, and Paxos's** own risk posture (issuer collateralization, governance, audits, regulatory standing) as the de-facto floor on USDat risk.
5. Reconcile whether **"Saturn Global Capital Investments Ltd."** (ToS-sourced) and **"Saturn Capital"** (DD-doc-sourced) are the same legal entity.
6. Pull a **historical peg/price series spanning the August 19, 2026 migration** and exact holder-concentration percentages (Etherscan Pro / Dune).
7. Confirm the **equivalent of `currentIndex()`** (reverts on the new implementation) for programmatic exchange-rate computation, if one exists.
8. Confirm whether **Saturn discloses the PYUSDx→PYUSD→Paxos attestation chain** to USDat holders anywhere in its docs.

### Sources consulted this session (September 8-10, 2026)

On-chain verification via `cast` against `RPC_1` (project `.env`, Ethereum), `https://rpc.monad.xyz` (Monad), and `https://bsc-rpc.publicnode.com` (BNB Chain); Etherscan API v2 (`ETHERSCAN_TOKEN` from project `.env`) across chain IDs 1, 56, and 143 for `getsourcecode` on the new USDat implementation, the M0 Swap Facility and its implementation, the BNB Chain/Monad USDat proxies and their `SaturnOFT` implementations, and the Chainlink CCIP token pools; the CCIP `TokenAdminRegistry` (`0xb22764f98dD05c789929716D677382Df22C05Cb6`) per the bridge-dependency skill's documented onchain-read procedure; full verified Solidity source (56 files for USDat, plus the SaturnOFT and pool contracts) retrieved via the Etherscan API and Sourcify v2 API and read directly for the mint/redeem/asset-replacement/bridge mechanics documented above. Earlier in this session, before `.env` was located, public RPC endpoints (`rpc.mevblocker.io`, `ethereum-rpc.publicnode.com`) were used for the initial discovery pass; findings were re-verified against the authenticated `RPC_1` endpoint where it mattered. Saturn's [key-addresses](https://saturncredit.gitbook.io/saturn-docs/operations-and-governance/key-addresses) and [transparency-and-audits](https://saturncredit.gitbook.io/saturn-docs/operations-and-governance/transparency-and-audits) GitBook pages; DefiLlama protocol TVL API; public web search for PYUSDx/MoonPay/Paxos background and for independent security disclosures (surfaced the Innora Security Research gist); Certora's public report page (confirmed April 30, 2026 date, no PYUSDx mention). A Playwright browser tool became available mid-session but its permission grant did not succeed despite repeated attempts, so GitBook's JS-rendered pages (audits list, Safe Harbor registry) were never fetched directly by this agent. **The fifth audit's filename, confirmation of no Safe Harbor adoption, a qualitative read of USDat's holder distribution (predominantly DeFi protocols), and a Terms-of-Service excerpt naming the operating legal entity were relayed directly by the report requester** (who had access to Saturn's own dashboard/site), not independently fetched or verified by this agent this session — treated as sourced information, but the underlying pages were not re-confirmed first-hand. Sources from the June 17, 2026 assessment (DD docs, prior audit PDFs) were not re-fetched.
