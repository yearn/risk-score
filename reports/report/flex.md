...
- **Oracles:** Each market has its own immutable Vyper oracle, and the v2 markets no longer share a single design:
...
Pseudonymous but Yearn-ecosystem-known lead dev; mechanics docs are accurate and were updated for v2; the allocator code and both new audits were published, including the unfavorable one. Against that: a **misleading "no admin keys" claim** that is now further from the truth, no governance or legal disclosure, no bug bounty, an undisclosed fifth Safe signer, and twelve findings left open on deployed code. → **3**
