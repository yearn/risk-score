...
- **Assessment Date:** June 19, 2026 (Updated: August 30, 2026)
...
- **TVL: ~$1.06M.** [DeFiLlama](https://defillama.com/protocol/flex) reports $1,058,877 for Flex on August 30, 2026. This reconciles with onchain collateral: $663,930 (yvUSD) + $243,076 (ysyBOLD) + $116,143 (yvcrvUSD-2) + $24,960 (retired v1 market) = **$1,048,108**.
...
The `yvFlexUSDC` vault holds **$1,111,699** and allocates it across four strategies — **81.0% of it sits in Flex**:

| Strategy | Address | Allocation | Underlying | Instantly withdrawable |
|----------|---------|-----------:|------------|----------------------:|
| Flex yvUSD/USDC Lender | [`0x7501EAE6…D737b`](https://etherscan.io/address/0x7501EAE6b5C2Cb0A6EDAC908E3A679B20eDd737b) | $600,358 (54.0%) | Flex Lender [`0xD93Dade7…e8bA`](https://etherscan.io/address/0xD93Dade7Ac8b5d1687da5d074835cB4404Dee8bA) | $7,203 |
| Flex ysyBOLD/USDC Lender | [`0xDd58AeFE…b5176`](https://etherscan.io/address/0xDd58AeFE74585DA2C7Ad3D3dF0F3aeEAb05b5176) | $200,007 (18.0%) | Flex Lender [`0xf4996Ca4…a03C`](https://etherscan.io/address/0xf4996Ca4190A1a3e7CF19AbE2F6eb712abd4a03C) | $0 |
...
| USDC To SKY USDS Depositor | [`0xfb4F83c3…F95e`](https://etherscan.io/address/0xfb4F83c3923eab7B6254cD2399C206109970F95e) | $211,334 (19.0%) | Sky sUSDS — not a Flex position | $211,432 |
...
|--------|--------------|--------|------------|-----:|-----------------:|----------:|-------:|
| yvUSD/USDC | [`0x8ee72c38…83c9`](https://etherscan.io/address/0x8ee72c388aA73096338EE18CD46a39D98b8983c9) | [`0xD93Dade7…e8bA`](https://etherscan.io/address/0xD93Dade7Ac8b5d1687da5d074835cB4404Dee8bA) | yvUSD | $593,541 | $663,930 | **111.9%** | 7 |
| ysyBOLD/USDC | [`0xADf4E022…Fc6E`](https://etherscan.io/address/0xADf4E0226d59aac20272023c04B4DcF5Ade7Fc6E) | [`0xf4996Ca4…a03C`](https://etherscan.io/address/0xf4996Ca4190A1a3e7CF19AbE2F6eb712abd4a03C) | ysyBOLD | $200,075 | $243,076 | **121.5%** | 7 |
| yvcrvUSD-2/USDC | [`0x7582b474…eA2E`](https://etherscan.io/address/0x7582b47486F75F5D675f260d357972cD0DbEeA2E) | [`0xc1f281A3…eC84`](https://etherscan.io/address/0xc1f281A3643F219636F97F6E687A33704950eC84) | yvcrvUSD-2 | $100,011 | $116,143 | **116.1%** | 5 |
| yvUSD/USDC (unendorsed) | [`0xd82DB989…2e49`](https://etherscan.io/address/0xd82DB9893751E9C90E2a6C3bE31183048E8E2e49) | [`0x33C45216…B732`](https://etherscan.io/address/0x33C45216E121E31f1a8CD24C7E9d0d0C9e29B732) | yvUSD | 147 wei | $24,960 | — | 0 |
...
The yvUSD Lender holds $7,203 of idle USDC; the ysyBOLD and yvcrvUSD-2 Lenders hold none. Lender withdrawals beyond idle cash call `TroveManager.redeem(...)`, which Dutch-auctions borrower collateral; proceeds route to the withdrawer.
...
- **Onchain and over-collateralized, with a thin but no longer uniform buffer.** Against a 110% MCR:
  - **yvUSD/USDC:** `total_debt` = **593,541.08 USDC**; `collateral_balance` = **646,616.05 yvUSD** priced at **1.026776 USDC/yvUSD** → **$663,929.84**; **system CR = 111.86%**.
  - **ysyBOLD/USDC:** `total_debt` = **200,075.28 USDC**; `collateral_balance` = **222,524.87 ysyBOLD** priced at **1.092354 USDC/ysyBOLD** → **$243,075.87**; **system CR = 121.49%**.
  - **yvcrvUSD-2/USDC:** `total_debt` = **100,010.83 USDC**; `collateral_balance` = **100,772.05 yvcrvUSD-2** priced at **1.152529 USDC/yvcrvUSD-2** → **$116,142.70**; **system CR = 116.13%**.
- **Trove-level distribution.** Walking all 19 Troves across the three markets gives a range of **110.02% to 139.22%** and a debt-weighted average of **113.17%**. The tail matters more than the average: **11.8% of debt sits below 111% CR, 31.5% below 112%, and 84.5% below 113%**. Per market the debt-weighted CR is 111.95% (yvUSD), 115.27% (ysyBOLD) and 116.14% (yvcrvUSD-2), so the largest market is also the thinnest — and one ysyBOLD Trove sits at 110.02%, effectively at the liquidation line. Because all three collaterals are yield-bearing USD vault shares whose price falls only on a realized loss, the practical trigger is an impairment in an underlying vault rather than market volatility; when it comes it hits a whole market's book at once.
- **First-loss reserve is negligible.** The v2 upfront fee is meant to buffer lenders against bad debt, but `unclaimed_protocol_fees` reads **$81.48**, **$23.06** and **$1.91** across the three markets — **$106.45 in total against $893,627 of debt**. The buffer is real in design and immaterial in size, and Dedaub advisory A1 plus Zero Cool H-01 and M-07 all describe ways for borrowers to suppress it further.
...
- **Exit is auction-mediated, and the standard path frees very little.** The yvUSD Lender holds $7,203 of idle USDC; the other two hold none. `FlexLenderStrategy.availableWithdrawLimit()` — idle USDC in the strategy plus idle USDC in its Lender — therefore returns **$7,203, $0 and $0** across the three Flex legs. Of the allocator vault's $1,111,699, only **$218,635 (19.7%) is instantly withdrawable**, and $211,432 of that is the non-Flex Sky USDS strategy. Freeing the $900,365 in Flex requires Yearn SMS to call `forceFreeFunds`, or a user to route through the `FlexExitRouter` — in which case idle liquidity is delivered atomically, the remainder arrives asynchronously as the redemption auction is taken, and the shortfall is **accounted as a loss on that user's withdrawal**.
- **Depth:** TVL is small (~$1.06M) and depositor concentration is extreme (90.9% Yearn Treasury). There is no external DEX market for `ysUSDC`; exit relies entirely on the protocol's own redemption machinery against $1,023,148 of collateral in the three live markets, and on the collateral vaults' own exit liquidity. Spreading the book across three collaterals is a mild diversification of that exit path, but each market's auction can only be taken in its own collateral.
- **Stress behavior:** Auctions run 1 day with 1-minute steps, starting at 100% of oracle value and stopping at 99%, so a taker willing to redeem the collateral vault share for its underlying can clear near par in benign conditions. Under stress the same auction becomes the throttle: redemptions and liquidations can deliver less than 1:1, bad debt is socialized to lenders, and `_harvestAndReport` is blocked while an auction remains unsettled. Zero Cool M-02 additionally describes a case where a redemption that hits the 1,000-Trove traversal bound books the entire unfilled amount as a loss — not reachable at 7, 7 and 5 Troves today, but a scaling constraint.
...
- **Backing ratio** per market = (`collateral_balance` × oracle price) / `total_debt`. Markets run close to the 110% MCR by design, so a static 112% alert would fire continuously. Alert if **system CR < 110.5%**; page if **< 110%**, on any `ReportLoss` on a Lender, or if the share of market debt held in Troves below 110.5% CR exceeds 50%. Track the debt-weighted CR per market rather than the system CR alone — the aggregate currently sits at 113.17% while 11.8% of debt is below 111%.
...
- **Collateral buffer has widened and diversified** — the debt-weighted collateral ratio across the book is 113.17% and the exposure now spans three markets rather than one, with the ysyBOLD market recapitalized to a 121.5% system CR.
...
- **The tail of the debt book sits on the liquidation line.** Against a 110% MCR, **11.8% of debt is below 111% CR, 31.5% below 112% and 84.5% below 113%**, with one Trove at 110.02%. The largest market (yvUSD, 66% of Flex debt) is also the thinnest at a 111.95% debt-weighted CR. The protocol's first-loss reserve against bad debt is **$106.45** across all three markets.
- **Very new & small** — the newest market is roughly a week old, TVL is ~$1.06M, and **90.9% of the depositor base is the Yearn Treasury itself** in a vault that is not a listed Yearn product.
- **Exit frees almost nothing through the standard path.** Only **19.7% of the allocator vault is instantly withdrawable**, and most of that is the non-Flex Sky leg; the three Flex strategies report $7,203, $0 and $0. Freeing the rest requires Yearn SMS action or an ExitRouter exit that delivers proceeds asynchronously and books the shortfall as the exiter's loss.
...
**Subcategory B: Historical Track Record** — The protocol is ~3.5 months old; its live markets are 3.5 weeks, 2.5 weeks and **one week** old respectively, with ~$1.06M TVL, 19 Troves, and 90.9% of the depositor base being the Yearn Treasury. Effectively no track record. → **5**
...
**Subcategory A: Governance** — Immutable market clones with non-custodial privileged control: Daddy cannot seize collateral or lender shares, and the Safe is 3-of-5. The August 24 transactions show the surface is live rather than theoretical — three registry actions and an oracle-parameter change, all effective immediately with no timelock. The choices made were sane and lender-protective, which is evidence about the operators rather than about the control structure; the structure still permits the reverse. Docs continue to deny that any of it exists. → **3**
...
**Score: 2.83/5** — Immutable mechanics and non-custodial governance, weighed down by an open share-pricing surface and a dependency chain that now reaches two unassessed protocols.

#### Category 3: Funds Management (Weight: 30%)

**Subcategory A: Collateralization** — Over-collateralized, onchain and real-time verifiable in high-quality DeFi collateral, spread across three markets with a debt-weighted collateral ratio of 113.17% and real dispersion (110.02%–139.22%). The buffer is still thin where it counts: 84.5% of debt sits below 113% CR against a 110% MCR, the largest market is the thinnest, the first-loss reserve is $106.45, and a liquidation fee of 0.5–5% eats into the margin before lenders are made whole. → **3.25**
...
**Optional Modifiers:** None apply (protocol < 2 years; TVL not sustained). Final score ≈ **3.16/5.0**.
...
- **Collateralization:** Reassess if any market's system CR falls below **110.5%**, if the share of a market's debt in Troves below 110.5% CR exceeds 50%, or if `unclaimed_protocol_fees` is materially depleted.
