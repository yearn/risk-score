...
- **Admin control over funds:** The immutable market contracts hold collateral; no admin can withdraw borrower collateral. Daddy (2/3 multisig) is the Lender's *management* and the Registry owner — it cannot seize collateral or user shares, but it can change Lender parameters, shut the Lender down, change the keeper, set fees, and endorse/unendorse markets (see *Centralization*).
...
- **No timelock** sits in front of Daddy. A **2/3 threshold with undisclosed signers and no timelock** is a low bar.
...
- **Keeper dependency:** Lender accounting (`report()`) is driven by a **permissionless keeper** (`0x52605Bbf…b2f8` hard-coded in `LenderFactory`); the Yearn allocator/strategy use Yearn's **yHaaS keeper** (`0x604e586F…711E`). The stale-PPS-between-reports interaction (above) is the main programmability caveat.
...
| Daddy owner Safe (2/3) | [`0x687b82dA9753C9db280d4D9aBD7BCAC022Ef3B67`](https://etherscan.io/address/0x687b82dA9753C9db280d4D9aBD7BCAC022Ef3B67) | signer/threshold changes |
...
- [ ] **Total centralization** — *Not triggered.* Core immutable; control via 2/3 Safe (not a lone EOA), and Daddy cannot seize funds.
...
**Subcategory A: Governance** — Immutable core, but a **2/3 multisig with no timelock and undisclosed signers** controls Lender params, fees, shutdown, keeper, and market endorsement; cannot seize funds. → **4**

**Subcategory B: Programmability** — Mostly programmatic; PPS onchain. Keeper-gated `report()` creates a documented stale-PPS window. → **2**
...
**Centralization Score = (4 + 2 + 3.5) / 3 = 3.17 ≈ 3.2**

**Score: 3.2/5** — Immutable mechanics, but low-threshold governance and a concentrated, self-referential collateral/oracle dependency.
