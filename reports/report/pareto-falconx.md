...
- **Final Score: 5.0/5.0**
- **Status:** GATED — score capped by the "uncollateralized / unverifiable reserves" critical gate (Category 3A rubric). Ungated weighted score is **3.98 (Elevated Risk)**. No realized loss event; the vault is live and current on all 13 epochs to date.
...
Pareto (formerly Idle Finance, code authored by "Idle Labs Inc." per the contract headers) supplies the vault infrastructure; **M11 Credit** (Maven 11) is the named curator/underwriter for this vault, per [Pareto's live-vaults page](https://docs.pareto.credit/product/credit-vaults/live-vaults). Deposits and redemptions are gated by Keyring KYC; the tranche token itself is a plain, freely transferable ERC-20, which is why it circulates as Morpho collateral even though redemption is permissioned.
...
| ProxyAdmin | [`0x9438904abC7d8944A6E2A89671fEf51C629af351`](https://etherscan.io/address/0x9438904abc7d8944a6e2a89671fef51c629af351) | Owns upgrade rights on CDO + strategy |
...
**Assessment of coverage.** Audit *cadence* is excellent — every material release since Aug 2024 has been reviewed, and the June 2026 review lines up with the implementation deployed on 2026-06-19. Audit *diversity* is the weakness: every credit-vault review from Aug 2025 onward is by the same individual researcher (0x52, via Sherlock), and the 2024 reviews are all by a single independent researcher (Hans Friese). There is no multi-firm review, no formal verification, and no public competitive audit contest for the credit-vault contracts. Reports are hosted on Google Drive rather than in a public repository, so historical findings are not diffable.
...
**Unresolved findings:** TODO — the audit PDFs are hosted on link-shared Google Drive and were not machine-readable during this assessment. Individual finding status could not be verified. Recommend requesting the fix-review letters directly from Pareto before integration.
...
**Interpretation.** There is no evidence of a loss: the redeeming lender appears to have been paid in full and slightly early. But the episode establishes an important precedent for how this product behaves at the edges — **when a vault is wound down, settlement moves off-chain and on-chain state is abandoned in a stale, misleading configuration**. An integrator relying purely on contract mechanics (`defaulted()`, `claimWithdrawRequest()`, receipt balances) would have been stranded: withdrawal requests remain permanently unclaimable, and the residual $17.6M AA position has no on-chain resolution. Whether the remaining Optimism holder was made whole is **unverified** — no public disclosure of the Optimism wind-down was found in Pareto's docs, blog, or governance forum (the Discourse search API returns zero results for "falconx").
...
No public default, insolvency, or credit event involving FalconX was found. **However:** FalconX's financial statements are not public, no covenant package or borrowing-base disclosure is published to on-chain lenders, and there is no independent attestation of the loan's performance. The $182.9M owed to this vault cannot be sized against FalconX's balance sheet from public information — **TODO for any integrator: request the credit agreement, financial covenants, and audited financials from M11 Credit / Pareto under NDA.**
...
- **Bypass allowlist:** `KeyringIdleWhitelist.setWhitelistStatus` is controlled by a **single EOA** ([`0xE5Dab…3e64B`](https://etherscan.io/address/0xE5Dab8208c1F4cce15883348B72086dBace3e64B)); 63 addresses have been whitelisted and **none revoked** (63 `Whitelist` grant events, 0 revocations), including the queue contracts, the Treasury Safe, and the two large Morpho loopers. This EOA can grant vault access to any address without KYC.

### Token Mint Authority

**Mint mechanism:** collateral-gated mint through the vault only. `IdleCDOTranche` (the AA token) hard-codes `minter = msg.sender` **in its constructor** — the CDO proxy that deployed it — and the field has no setter. There is no `AccessControl`, no `Ownable`, no role registry, and no upgrade path on the token itself (it is a plain immutable contract, not a proxy).

**Mint requires backing:** Yes, with one nuance. Every mint path (`_deposit`, `depositDuringEpoch`, and the fee-share mint inside `stopEpoch`) mints against underlying USDC that has entered the system in the same transaction, or against interest the CDO has already booked. The nuance is that "backing" here means *a receivable from FalconX*, not custody of an asset: after `startEpoch`, the CDO's own USDC balance is zero.
...
|---------|:--------:|:--------:|------------------|-------|
| [`0x433D5B175148dA32Ffe1e1A37a939E1b7e79be4d`](https://etherscan.io/address/0x433D5B175148dA32Ffe1e1A37a939E1b7e79be4d) | ✓ | ✓ | `minter` (immutable, set in constructor) | The Credit Vault (CDO) proxy — the **only** address that can mint or burn AA_FalconXUSDC. Its logic is upgradeable behind a 24h timelock, so the *rules* governing minting can change even though the minter address cannot |

No other address holds mint authority. The LayerZero adapter does **not**: `FalconXAAAdapter` [`0x8FEd…667e`](https://etherscan.io/address/0x8FEd610aEfD81F6bEC517854b1a245DEca83667e) is an OFT **Adapter** (`approvalRequired()` = `true`, `token()` = the AA token) that escrows canonical tokens; it cannot mint them, because the token's minter is the CDO and is immutable. Cross-chain supply on Monad is a bridged representation, not native supply.

**Rate limits / supply caps:** none. `limit()` = 0 (no TVL cap, i.e. `_guarded` is a no-op) and `maxApr()` = **0 on the strategy, which disables the APR cap entirely** (`DEFAULT_MAX_APR` of 20e18 was overridden). There is no per-epoch mint ceiling.

**Backing check at mint time:** atomic in the sense that USDC must be transferred in the same transaction — but the USDC is immediately forwarded to an off-chain counterparty, so "backing" degrades to an unsecured receivable within the same block for mid-epoch deposits.
...
- **What *is* provable on-chain:** the outstanding principal (`IdleCreditVault.totalSupply()` = $182,850,264.50), the split between live NAV and unfunded withdrawal receipts, the borrower's address, every historical repayment (`AccrueInterest`), and the exact tranche price. The exchange rate is computed programmatically in `virtualPrice()` from booked NAV — no privileged oracle push is involved.
...
- **Accounting is admin-driven at the boundary.** `stopEpoch(_newApr, _interest)` lets the manager **override** the epoch's realized interest with an arbitrary `_interest` value, and `stopEpochWithDuration` adds an arbitrary `_lossAmount`. There is no on-chain constraint tying either to observable reality; with `maxApr() = 0`, the sanity cap that would bound an overridden interest figure is disabled.
...
**Upgradeability.** Both the CDO and the strategy are `TransparentUpgradeableProxy` contracts behind ProxyAdmin [`0x9438…f351`](https://etherscan.io/address/0x9438904abc7d8944a6e2a89671fef51c629af351), owned by the OZ `TimelockController` [`0xDa86…5A44`](https://etherscan.io/address/0xDa86e15d0Cda3A05Db930b248d7a2f775e575A44) with `getMinDelay()` = **86,400 s (24 hours)**. Role enumeration from `RoleGranted`/`RoleRevoked` logs, confirmed with `hasRole`:
...
- `IdleCreditVault.setMaxApr`, `setManager`, `setWhitelistedCDO`; `IdleCDO.setFeeParams`, `setGuardian`, `setLossToleranceBps`, `setIsAYSActive`, `_setLimit`, `setIsInterestMinted`, `setIsProgrammableBorrower`, `restoreOperations`.
- **Guardian (2-of-4 Safe)**: `pause()`, `emergencyShutdown()`, `updateAccounting()` — can halt deposits and redemption requests at will.
- **Keyring bypass allowlist** is administered by a **single EOA**, which can grant or revoke vault access.

**Can governance seize user funds?** Not from a wallet — the tranche token has no clawback and no blacklist. But the owner can (a) sweep the strategy's USDC to an arbitrary address, (b) redirect future principal to a new borrower, (c) write down NAV via `stopEpochWithDuration(_lossAmount)` or `updateAccounting()`, and (d) freeze redemptions indefinitely via the guardian. Collectively, a 3-of-8 multisig with no timelock is sufficient to cause total loss.

### Programmability

The system is **manually operated by design**. Every epoch boundary is a privileged call:

- `startEpoch()` and `stopEpochWithDuration(newApr, interest, duration, loss)` are callable by the owner or the **manager**, which is the `IdleCreditVaultManagerOrchestrator` [`0x052e…323b`](https://etherscan.io/address/0x052e51568351eFfa9EA90DF6FE648446f006323b). Its `operator` is a **single EOA keeper**, [`0x1fb0…Df58`](https://etherscan.io/address/0x1fb0f3602F52e2420aCff5CF04DBfDE96378Df58) — the same keeper address used on the Optimism deployment. That one key can start and stop epochs, set the next epoch's APR (uncapped, since `maxApr()` = 0), override reported interest, and burn strategy tokens as a "loss".
...
| Keyring (policy 18) | High for access | If the credential registry misbehaves or the policy changes, deposits and redemption requests revert for affected wallets |
...
- The oracle is Pareto's own accounting price — it cannot mark down before Pareto does, so the LLTV buffer protects against *administrative* markdowns, not against a silent credit deterioration.
...
**Path B — deposit USDC directly into the FalconX credit vault to earn ~8%.** This makes Yearn an unsecured lender to FalconX with a 29–60 day exit, and it requires **Keyring policy-18 KYC (or an admin allowlist entry) for the depositing address**, plus a $250K minimum. Every risk in this report applies at full weight, with no LTV buffer and no seniority. A Yearn v3 strategy wrapping this asset would also have to model: a share price that only moves at epoch boundaries, a redemption that cannot be served on demand, and a `report()` that would be marking an unsecured receivable at par.
...
| Tranche price integrity | CDO | `virtualPrice(AA)` vs `priceAA()` | Any **decrease** → critical (first-ever markdown) | Hourly |
...
| Upgrades | ProxyAdmin [`0x9438…f351`](https://etherscan.io/address/0x9438904abc7d8944a6e2a89671fef51c629af351), Timelock [`0xDa86…5A44`](https://etherscan.io/address/0xDa86e15d0Cda3A05Db930b248d7a2f775e575A44) | `Upgraded(address)` (topic0 `0xbc7cd75a…2d3b`); timelock `CallScheduled` | Any schedule → review within the 24 h window (note: exit is impossible in that window) | Every block |
...
| KYC gate | Keyring wrapper [`0x6a6A…50e3`](https://etherscan.io/address/0x6a6A91c7c7C05f9f6B8bC9F6e5eA231e460450e3) | `isWalletAllowed(<our address>)`, `Whitelist(address,bool)` events, `admin()` | Our address losing access → critical; `admin` change → warning | Daily |
...
| Off-chain counterparty | — | FalconX news flow: IPO/S-1 progress, credit-rating or funding events, regulatory actions | Any adverse credit headline → immediate reassessment | Continuous |
...
└───────────┬──────────────────┘                                     │ ProxyAdmin         │
            ...
            │  • transferToken(token, amt, ANY dest)  ◀── sweep      └─────────┬──────────┘
            │  • setBorrower / setMaxApr / setFeeParams                        │ upgrades
            │                                                                  │ (5 upgrades
┌───────────▼──────────────────┐   pause / emergencyShutdown /                  │  in 13 mo)
│ Pauser Safe 2-of-4           │──▶ updateAccounting                            │
│ 0xBaeC…Bb9f  (guardian)      │                                                │
└──────────────────────────────┘                                                │
┌──────────────────────────────┐  operator EOA 0x1fb0…Df58                      │
│ ManagerOrchestrator          │──▶ startEpoch / stopEpochWithDuration           │
...
│   deposit/redeem gate ──▶ Keyring 0x6a6A…50e3 (policy 18) ──▶ KeyringCore 0xb0B5…e666│
│   USDC balance: $0        NAV: $167,613,658.26        fee: 10% → Treasury            │
└───┬─────────────────────────┬───────────────────────────────┬───────────────────────┘
...
│ supply 152,088,542   │  │ supply 0 — NEVER     │  │ receipt token, 1:1 w/ USDC        │
│ price 1.102079       │  │ USED (no junior      │  │ supply $182,850,264.50            │
│ freely transferable  │  │ protection at all)   │  │ USDC balance: $0                  │
└───┬──────────────────┘  └──────────────────────┘  │ pendingWithdraws $15,236,606.24   │
    │                                                └────────────────┬──────────────────┘
    │                     ┌──────────────────────┐                    │ 100% of principal
    ├────────────────────▶│ EpochQueue 0x5cC2…95DC│                   │ transferred at
    │  queued deposits /  │ (KYC-gated, operator- │                   │ startEpoch
...
- **On-chain, programmatic share price:** `virtualPrice()` is computed from booked NAV rather than pushed by an oracle, and the loan's principal, repayment history, and redemption backlog are all publicly readable.
- **Real governance hygiene where it is applied:** upgrades genuinely go through a 24 h timelock proposed by a 3-of-8 Safe, the deployer's PROPOSER role was revoked, and the token itself is immutable with a single, unchangeable minter.
...
- **Timelock-free owner powers.** A 3-of-8 Safe can sweep the strategy's USDC to an arbitrary address (`transferToken`), change the borrower, and rewrite fee parameters with no delay; a 2-of-4 Safe can freeze redemptions; a single EOA keeper sets the APR (cap disabled, `maxApr()` = 0) and can burn NAV as a "loss".
- **Exit is 29–60 days, KYC-gated, and borrower-funded.** There is no DEX liquidity and no on-chain reserve; every redemption depends on FalconX wiring cash at an epoch boundary. A large simultaneous exit has never been tested.
- **Extreme concentration on both sides.** Top four holders = 87% of supply; 38.7% is levered through one Morpho market where two positions hold 97% of the debt at HF ~1.11–1.13.
...
- **"No default" is partly an artifact of the mechanism.** `defaulted` is only set when a curator-initiated repayment pull reverts. On the **superseded Optimism vault, the same borrower's epoch expired on 2025-07-31 and `stopEpoch` was never called** — leaving `defaulted() == false`, $17.57M of AA NAV and $10.60M of withdrawal receipts stranded on-chain for 370+ days, with the $10.6M redemption settled by a **direct off-chain USDC transfer** from the borrower to the lender on 2025-07-10. A missed payment can therefore present as silence, not as an on-chain default.
- **The Morpho oracle is Pareto's own accounting price** (`TranchesChainlinkOracle` → `cdo.virtualPrice()`), so collateral cannot be marked down by the market — only by the vault's administrators. With two borrowers at HF ~1.11–1.13 holding 97% of $44.3M of debt, a single ~10–12% administrative markdown makes nearly the whole market liquidatable at once, into an asset that **non-KYC liquidators cannot redeem and cannot sell**. Bad debt for USDC suppliers is the plausible outcome, not liquidation.
...
**Subcategory A — Audits & Security Reviews: 3.0.** Eleven engagements covering every material release, tracking the deployed implementation closely — but concentrated in **two individual reviewers** (0x52 via Sherlock for everything since Aug 2025; Hans Friese for 2024), with no multi-firm review, no formal verification, no public contest, and reports published only as Drive links. The bug bounty maxes at **$50K** against $182.9M at risk (0.027%), which sits in the rubric's "minimal bounty" band. Contract surface is a 939-line epoch state machine with several dormant-but-reachable accounting modes.

**Subcategory B — Historical Track Record: 2.0.** ~13.5 months in production (June 18, 2025), TVL sustained above $100M since roughly March 2026 and now $167.6M, thirteen clean epochs. The counterparty relationship predates this vault (Optimism, 2024–2025). Held back from 1.0 by the vault's age and by the unresolved on-chain state of the predecessor deployment.

**Audits & Historical Score = (3.0 + 2.0) / 2 = 2.5/5**

#### Category 2: Centralization & Control Risks (Weight: 30%)

**Subcategory A — Governance: 4.0.** Upgrades are timelocked at 24 h and proposed by a 3-of-8 Safe (good), but the **owner powers that can move money are not timelocked at all**: `transferToken(token, amount, arbitrary destination)` on the strategy, `setBorrower`, and the full fee/parameter surface are direct 3-of-8 Safe calls. A 2-of-4 Safe can freeze redemptions. A single EOA administers the KYC bypass allowlist. Five implementation upgrades in 13 months against a redemption queue of 29–60 days means the timelock provides **no exit window** for lenders. Rubric: "Multisig 3/5 or low threshold, <12 h, powerful admin roles with limited constraints."

**Subcategory B — Programmability: 4.0.** Every epoch boundary is a manual privileged call by a single EOA keeper, which also sets the next epoch's APR with the cap disabled (`maxApr()` = 0), can override reported interest, and can burn strategy tokens as a realized loss. Repayment depends on the borrower granting an allowance off-schedule (currently 0 mid-epoch). Offsetting this, share price is computed on-chain rather than pushed. Rubric: "Significant manual intervention required."
...
**Centralization Score = (4.0 + 4.0 + 5.0) / 3 = 4.33/5**
...
**Subcategory B — Provability: 4.0.** Genuinely strong on the *liability* side — principal, backlog, repayment history and share price are all on-chain and programmatic. Genuinely absent on the *asset* side — no attestation, no reserve report, no borrower disclosure, and administratively supplied interest/loss inputs. Rubric: "Primarily offchain / infrequent reporting / self-reported only."

**Funds Management Score = (5.0 + 4.0) / 2 = 4.5/5**
...
| **Ungated weighted score** | | | **3.98/5.0 (Elevated Risk)** |
...
- **Structure-based:** reassess if a junior (BB) tranche is funded (materially *reduces* risk), if `setBorrower` is called, if `maxApr` is re-enabled or the APR moves more than 300 bps, or if the owner's un-timelocked `transferToken` power is moved behind the timelock.
...
- **Incident-based:** immediate reassessment on any `BorrowerDefault` emission, any non-zero `_lossAmount`, any decrease in `virtualPrice`, any epoch running more than 24 h past `epochEndDate`, or any bad debt in the Morpho market.
- **Resolution of the Optimism precedent:** if Pareto publishes an account of the Optimism wind-down, or the residual $17.57M position is settled on-chain, revisit the Operational and Historical scores.

## Assessment History

| Date | Score | Notes |
| --- | --- | --- |
| August 5, 2026 | 5.0 (GATED) | Initial assessment. Gate: uncollateralized / unverifiable reserves. Ungated weighted score 3.98 (Elevated). 13/13 epochs repaid; $167.6M senior NAV; no junior tranche. |
