...
**Subcategory B — Provability: 4.0.** Genuinely strong on the *liability* side — principal, backlog, repayment history and share price are all on-chain and programmatic. Genuinely absent on the *asset* side: no attestation, no reserve report, administratively supplied interest/loss inputs, and — now confirmed against SEC EDGAR — **no borrower financial disclosure exists anywhere on public file** (only Form D notices marked "Decline to Disclose"; the S-1 remains confidential). The one indirect proxy a lender might use instead, arm's-length deposit demand, is also compromised: ~16% of all AA ever minted was funded from a FalconX-side settlement wallet. Rubric: "Primarily offchain / infrequent reporting / self-reported only."
