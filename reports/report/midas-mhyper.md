...
- **NAV/Price Updates:** Token price updated **twice per week** by Midas via a privileged role on the `CustomAggregatorFeed` oracle. Current price: ~$1.089 (round 79, 8 decimals). The oracle enforces onchain bounds: `maxAnswerDeviation` of 35% (max price change per update), `minAnswer` of $0.10, and `maxAnswer` of $1,000. These prevent extreme single-update manipulation but still allow significant price swings (~35% per tx). The exchange rate is determined offchain; users cannot know the exact amount of tokens they will receive beforehand (flagged by auditors). The Attestation Engine separately verifies and anchors NAV source-data hashes, but does not currently enforce oracle update correctness onchain
...
- **DEFAULT_ADMIN_ROLE holders:** Three addresses hold `DEFAULT_ADMIN_ROLE` on MidasAccessControl — **role changes (mint/burn/pause/blacklist grants) bypass the timelock** and can be executed immediately:
  ...
  - EOA [`0xd4195cf4df289a4748c1a7b6ddbe770e27ba1227`](https://etherscan.io/address/0xd4195cf4df289a4748c1a7b6ddbe770e27ba1227)
  - EOA [`0x875c06a295c41c27840b9c9dfda7f3d819d8bc6a`](https://etherscan.io/address/0x875c06a295c41c27840b9c9dfda7f3d819d8bc6a)
  - **Any one of these three can grant/revoke any role with no timelock.** The MPC/nested Safe mitigations above apply to the 1/3 Safe's owner set; unless separately confirmed by Midas, the two direct admin EOAs should be treated as EOAs onchain
...
- **Privileged Roles:**
  1. **`M_HYPER_MINT_OPERATOR_ROLE`** (`0xe6046a6e8c55ddf579e30dbcefd2018a368c8b9d4836e839e4858921fb6305d7`) — Can mint unlimited mHYPER tokens. **The `mint()` function has no onchain collateral check** — it only verifies the caller has the mint role, then calls OpenZeppelin `_mint()` directly. Currently held by:
    - EOA [`0x5683de280d0c3967fba2f04d707fa1ef5a044e25`](https://etherscan.io/address/0x5683de280d0c3967fba2f04d707fa1ef5a044e25) — can mint unbacked tokens anytime. Also holds BURN, PAUSE, BLACKLIST, and GREENLIST roles (5 total, all mHYPER-only). Funded via Midas deployer chain. Nonce 0 (never sent a transaction). **Not documented in [Midas security docs](https://docs.midas.app/security/smart-contract-security)**; if MPC-controlled, that is not labeled on Etherscan or verifiable onchain. All roles [granted](https://etherscan.io/tx/0x4254a910f7133d2ef54913662f19a0af1f9974d33391ca84ca8c60dffa025301) in a single batch on July 16, 2025. Verify: `cast call 0x0312A9D1Ff2372DDEdCBB21e4B6389aFc919aC4B "hasRole(bytes32,address)(bool)" 0xe6046a6e8c55ddf579e30dbcefd2018a368c8b9d4836e839e4858921fb6305d7 0x5683de280d0c3967fba2f04d707fa1ef5a044e25` → `true`; `cast code 0x5683de280d0c3967fba2f04d707fa1ef5a044e25` → `0x` (EOA)
...
│  ├─ EOA 0xd4195cf4df289a4748c1a7b6ddbe770e27ba1227                 │
│  ├─ EOA 0x875c06a295c41c27840b9c9dfda7f3d819d8bc6a                 │
