...
**Yield Mechanism:**
reUSD accrues yield daily. The base rate is a **blended rate** — a weighted average of (i) **deployed** (offchain) capital earning the risk-free rate (SOFR) and (ii) **undeployed** (onchain) capital capturing the Ethena USDe hedged basis trade — plus a **250 bps spread**. Re's docs state the spread per component ("SOFR plus 250 bps" on deployed capital; "7-day trailing average of the sUSDe basis-trade rate plus 250 bps" on undeployed capital, per [docs.re.xyz](https://docs.re.xyz/products/about-reusd)), which is arithmetically identical to adding 250 bps once to the weighted blend. It is **not** a "higher-of" floor between two independently-spread paths, though the NAV formula does take the higher of the sUSDe basis return and the T-bill return on the undeployed leg (see Funds Management → Token Mechanism).

The resulting "Applicable APY" is converted to a daily rate, and reUSD's **token price** (not quantity) increases daily — per docs, the price updates at 00:00 UTC subject to a daily move cap (onchain, the Chainlink Automation write is observed at ~23:45 UTC). Current APY is approximately 6.5–7% (mid-2026). See Funds Management → Token Mechanism for how the price is written onchain.
...
**Onchain re-verification (Jul 29, 2026):** `getSharePrice()` = `1.0917`; Ethereum `totalSupply()` ≈ **152.3M reUSD** (Ethereum NAV ≈ $166.3M). Supply is down ~14M since Jun 21 — consistent with the contracting-float observation in Liquidity Risk. Reserve balances in the tables below are as of Jun 21, 2026 unless noted.
...
A **Solana** deployment (integration with Kamino, priced via a Chainlink redemption-rate feed) has been announced as upcoming per Re team communications (Jul 2026). TODO: verify the token address and oracle feed once live.
...
| 2 | Nov 2024 | Smart Contract Audit | Hacken | Follow-up audit, issues remediated | [Hacken](https://hacken.io/audits/re-protocol/sca-re-re-contracts-nov2024/) |
| 3 | Mar 2025 | NAV Oracle Audit | Hacken | Scope: the Chainlink-Functions-based `NAVConsumer` + related code at `github.com/resilience-foundation/nav-oracle` (commits `ee7e98…` / `e3dd86ef…`). 8 findings, all resolved. The audited contract IS deployed and active onchain at [`0x84d4eaeb10f9e57b67622f667c6c13e22fa4b2b6`](https://etherscan.io/address/0x84d4eaeb10f9e57b67622f667c6c13e22fa4b2b6), holds `PRICE_SETTER_ROLE` on the `SharePriceCalculator`, and runs daily at 23:45 UTC. | [Hacken](https://hacken.io/audits/re-protocol/sca-re-nav-oracle-mar2025/) |
...
Dates in rows 1–3 follow the published Hacken report pages (slugs `aug2024`, `nov2024`, `mar2025`). Re's docs list the same engagements as "Sept 2024 / Dec 2024 / Apr 2025" (likely publication rather than report dates).
...
- **Production History**: Re Protocol launched in late 2022. reUSD token inception June 12, 2025 (per RWA.xyz). Curve pool created ~September 2025 per GeckoTerminal.
...
2. **Re Capital** (~$77M as of June 2026 per [docs.re.xyz](https://docs.re.xyz/products/about-reusd); ~$73M in the Nov 2025 LP memo — the reinsurer's equity) — **first-loss** buffer, absorbs losses in the **105–110%** band.
...
**Docs discrepancy (checked Jul 29, 2026):** Re's [Loss waterfall order](https://docs.re.xyz/capital-stack-and-loss-waterfall/loss-waterfall-order.md) docs page headlines *"Impairment of reUSD would require Re to operate at a Combined Ratio of 135%. That is a 0.03% probability event"*, while its own [Scenario walkthroughs](https://docs.re.xyz/capital-stack-and-loss-waterfall/scenario-walkthroughs.md) page confirms the band structure used here (*"premiums and the buffer cover claims to 105%, Re capital absorbs from 105% to 110%, reUSDe from 110% to 115%, and reUSD only above 115%"*). This report keeps the **115% attachment (~0.9%)** for reUSD, consistent with the scenario page and Re team guidance; the 135%/0.03% figure is a deep-stress level, not the attachment point.
...
- Redemptions are quarterly (requests submitted during the announced window — previously documented as the first 72h of each fiscal quarter), pro-rata if requests exceed available surplus; unfilled rolls to next quarter. Per current docs (Jul 2026), payouts are made **in sUSDe** and must be claimed within a **30-day** period
...
- reUSD is stated to be protected by subordinated assets (Re Capital + reUSDe combined); only Re Capital (~$73M in the LP Memo; ~$77M as of June 2026 per docs) is directly referenced. The total subordinated buffer amount is a protocol claim, not independently verified.
...
- **reUSDe — redemption works differently** (per [docs](https://docs.re.xyz/products/about-reusde)): **no instant redemption path exists**. reUSDe redemptions are quarterly-only, via a four-step process (submit during the announced window → window closes and NAV locks → pro-rata fill against *Available Surplus* → claim payout **in sUSDe** within **30 days**); unfilled balances auto-roll into the next quarter while retaining queue seniority. (Earlier docs specified the first 72 hours of each fiscal quarter as the request window and an "actuarial gate" at quarter-end of ≤10 business days.) Re explicitly notes *"No secondary market maker pool is promised"* for reUSDe. The senior-tranche instant buffer/vault described above applies to reUSD only, not reUSDe.
...
- **Re Capital buffer**: ~$77M (June 2026, docs) subordinated **first-loss** layer (the reinsurer's equity) ahead of reUSDe and reUSD; absorbs losses in the 105–110% combined-ratio band
...
3. **DEX Swap**: Sell reUSD on DEXes — primarily the Fluid reUSD/USDT pool (~$11.6M, the dominant venue); smaller Curve reUSD/sUSDe and reUSD/USDC pools
...
- **Multi-chain**: Available on 6+ chains. Liquidity concentrated on Ethereum — Fluid reUSD/USDT (~$11.6M) plus Curve (~$1.9M combined) — with ~$1.5M on Avalanche.
...
- **No onchain governance**: Protocol is currently governed by an expert-led council (Resilience Foundation). A **$RE governance token** was announced May 2026 ([press release](https://www.globenewswire.com/news-release/2026/05/26/3300870/0/en/Resilience-Foundation-to-Launch-The-RE-Governance-Token.html)); per [docs](https://docs.re.xyz/governance-and-tokenomics/re-governance.md), launch scope is "Phase 1: Foundational Governance". As of Jul 29, 2026 the onchain admin roles remain with the Safe/Timelock/MPC structure described above — $RE does not yet control the contracts.
...
- **Reinsurance tail risk**: Underlying assets are exposed to insurance claim risk. **reUSD is impaired once the portfolio combined ratio exceeds 115%** (its attachment point), after Re Capital (~$77M, first-loss, 105–110%) and all reUSDe reserves (mezzanine, 110–115%) are depleted. Re's model puts reUSD's loss likelihood at **~0.9%** (probability of reaching 115%); 135%/0.03% is a deeper-stress level, not the attachment. Re's historical combined ratio is ~92% and the portfolio avoids catastrophe lines, but a ~0.9% modeled attachment is not negligible (all figures Re-asserted, model undisclosed).
...
So Re's claim *"A JSON price feed is pushed on-chain via Chainlink"* is correct in a loose sense: the NAV is produced by Chainlink Functions and pushed by Chainlink Automation, even though it's not a classic Chainlink "price feed aggregator". The NAV Oracle code was audited by Hacken in Mar 2025 (repo `github.com/resilience-foundation/nav-oracle`).
