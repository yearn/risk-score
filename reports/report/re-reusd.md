...
- **Updated:** Combined reassessment — merges (a) the onchain governance/reserve/TVL refresh (Jun 21) and (b) team-feedback factual corrections: reUSDe tranche label & yield, reUSD yield mechanism, issuer entity, premium/portfolio figures, underwriting leadership, the loss-waterfall attachment points, and the **Chainlink Proof-of-Reserve finding (a live PoR feed was confirmed on Avalanche)**.
...
- **Final Score: 3.5/5.0** (Elevated). Was 3.6 in April; the Jun-21 onchain governance improvements pulled it down, but team-feedback corrections — reUSD-only onchain coverage sitting at the ~50% floor (~50.2%) and reUSD attaching at a 115% combined ratio (~0.9% modeled loss likelihood, ~30× the previously-framed 0.03% at 135%) — hold it at the Elevated/Medium boundary. See Risk Score Assessment.
...
- **Onchain reserves still concentrated in sUSDe but improving**: Onchain reserves total ~$93.66M (~51.9% of Ethereum NAV — the protocol-stated ≥50% target is met with ~1.9 pp of headroom). ~72.9% of reserves is sUSDe (down from ~88%) with ~20.9% in USDC ($19.55M, up from ~9.3%). Ethena (sUSDe issuer) remains a material counterparty, though the improved USDC share provides better immediate redemption coverage. The BUIDL / T-bill wrappers mentioned in Re's materials are **not** held onchain (Jun 21, 2026).
...
- **DEX liquidity thin**: only ~$14.96M of onchain-verified Re reUSD liquidity on DEXes (~8.2% of ~$181.8M market cap). (Previously-cited "~$26.2M" figure included Resupply reUSD pools, a different token.)
...
**Governance Score: 3.5** (was 4.0) -- Safe-backed Timelock and 48h delay are positive. Recent governance improvements (all verified onchain) materially reduce the *default* attack surface: the EOA bypass of the NAV Oracle is closed, custodian management is timelock-gated, and OFT adapter ownership is with the multisig. The improvement is tempered because the role admins for the price-write and mint paths are the 3-of-5 Governance Safe (not the Timelock), so `PRICE_SETTER_ROLE` and `MINTER_ROLE` can be re-granted without a 48h delay — the fixes are current-state, not structurally irreversible. Remaining concerns: (a) offchain-only MPC admin EOAs with unverifiable N-of-M quorums, (b) no onchain governance / DAO, and (c) KYC-gated access. Net: a 0.5-point reduction (not the full 1.0), since the largest fund-loss vector — the reserve-custody EOAs — is unchanged (and captured under Collateralization).
...
**Programmability Score: 3.75** (was 4.0) -- The core value-determining function (reUSD price/yield) is set by an admin-controlled oracle, not computed programmatically onchain. The EOA bypass of the NAV Oracle has been closed — the audited Chainlink-Functions path is the sole onchain price writer today, the NAVConsumer's `DEFAULT_ADMIN` is the Governance Safe, and the emergency role was revoked from the former EOA. Capital deployment is entirely offchain. The improvement is partial (0.25 point, not 0.5) because the `SharePriceCalculator`'s role admin is the 3-of-5 Safe (not the Timelock) and can re-grant `PRICE_SETTER_ROLE` without a timelock delay — the price path is not yet structurally tamper-resistant.
...
**Score: 3.75/5** (was 4.0) -- Centralization concerns are moderated by recent governance improvements (all verified onchain): the EOA bypass of the NAV Oracle is closed, custodial management is timelock-gated, and OFT adapter/minter-burner ownership is with the Governance Safe. The improvement is tempered because these fixes are re-grantable by the 3-of-5 Safe without a timelock delay (the price-write and mint role admins are the Safe, not the Timelock). Residual concerns: offchain-only MPC admin EOAs, offchain capital deployment, KYC gating, and heavy reliance on external entities.
...
**Collateralization Score: 4.25** (raised from 4.0) -- Hybrid onchain/offchain model. Onchain reserves verified at ~$93.66M (all 15 AUP-listed Fireblocks addresses) vs ~$180.55M Ethereum NAV (~51.9% coverage). Per team feedback, one address (~$3.05M) is the **reUSDe** ICL custodian; **excluding it, reUSD-only coverage is ~50.2% — only ~0.2 pp above the ≥50% floor** (the ~51.9% all-in figure has ~1.9 pp headroom). **Critically, ~$80.95M of the onchain reserve (~86%) sits at three plain EOAs with no onchain restriction on outflow** — a single signed tx can drain them; the 48h Timelock does not apply. The AUP corroborates the Fireblocks address list only, not the N-of-M quorum. ~72.9% of the reserve is sUSDe (Ethena counterparty risk, 7-day cooldown); USDC share has improved to ~20.9%. No BUIDL / T-bill wrappers are held onchain. Offchain reinsurance capital release relies on third-party attestation and is outside onchain verification. **Raised to 4.25** because two facts surfaced in this reassessment worsen the effective backing quality versus the prior 4.0: (a) reUSD-only onchain coverage is essentially at the protocol's own 50% floor (~0.2 pp headroom), not the ~1.9 pp the all-in figure implies; and (b) the subordinated buffer protecting reUSD attaches at a 115% combined ratio (~0.9% modeled likelihood), ~30× more likely to be reached than the 135%/0.03% framing earlier drafts used. The ~$81M plain-EOA custody surface remains the dominant, unchanged fund-loss vector.
...
**Provability Score: 3.75** (was 4.0) -- Re's reserve-publication-via-Chainlink claim is substantiated: a live Chainlink Proof-of-Reserve feed ("Re Offchain Reserves") runs on Avalanche, though it is a transparency mirror of TNF's offchain attestation and is not consumed by reUSD's Ethereum pricing/redemption logic (see the Chainlink usage appendix). Onchain Chainlink usage is (a) the sUSDe/USD price feed for collateral pricing, (b) Chainlink Functions + Automation driving `NAVConsumer` for the share-price update, and (c) the Avalanche PoR feed. The EOA bypass of the share-price path has been closed — `PRICE_SETTER_ROLE` is now held only by `NAVConsumer`, `DEFAULT_ADMIN` is the Governance Safe, and `EMERGENCY_UPDATER_ROLE` was revoked from the former EOA. MINTER_ROLE holders reduced from 3 to 2. Core yield calculation, reserve attestation, and performance reporting remain offchain with no independent onchain oracle to cross-check. The improvement is partial (0.25 point, not 0.5) because the share-price and mint role grants are re-openable by the 3-of-5 Safe without a timelock delay.

**Funds Management Score = (4.25 + 3.75) / 2 = 4.0**

**Score: 4.0/5** -- Hybrid model with meaningful offchain components. Three plain EOAs hold ~86% of onchain reserves with no onchain outflow restriction (custody risk remains the primary concern, unchanged since April). Core reserve provability relies on offchain attestation with no independent onchain oracle (the Avalanche PoR feed is a transparency mirror, not a control). The EOA oracle-bypass closure and MINTER_ROLE holder reduction improve Provability to 3.75, but Collateralization rises to 4.25 because the corrected reUSD-only coverage sits at the ~50% floor and reUSD's loss-attachment is ~30× more likely than earlier framing — netting the subcategory back to 4.0.
...
**Score: 3.75/5** (raised from 3.5) -- Onchain-verified DEX liquidity for Re reUSD is ~$14.96M (~8.2% of ~$181.8M market cap), of which ~78% sits in a single Fluid reUSD/USDT pool. Instant redemptions currently pay out in sUSDe only: ~$12.71M in the daily vault and ~$52.95M in the Redemption Reserves Custodian cover ~$65.7M of sUSDe-denominated exits under 20% daily / 10% per-wallet caps. Combined with DEX depth, protocol-native exits plus DEX can absorb ~$80.7M of the ~$181.8M outstanding before falling back to the quarterly queue. **Raised to 3.75** because token-level 24h volume has fallen sharply (~$7.3M → ~$1.4M) and reUSD's float is contracting (supply down ~172.6M → ~166.4M while protocol-wide TVL grew), thinning real spot-market exit depth on top of the pre-existing concerns: KYC-gated protocol redemptions, DEX liquidity concentrated in a single Fluid pool, and no USDC instant exits.
...
**Final Score: 3.5** (3.5125 rounded; was 3.6 in April 2026). The Jun-21 governance improvements (Centralization 4.0→3.75) are offset by the team-feedback corrections that worsen Funds Management (Collateralization 4.0→4.25 on reUSD-only coverage at the ~50% floor + a 115% attachment) and Liquidity (3.5→3.75 on the volume/float contraction), holding the protocol just inside Elevated.
