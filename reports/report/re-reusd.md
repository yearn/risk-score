...
- **Assessment Date:** September 7, 2026 (Updated: September 23, 2026)
...
- **Final Score: 3.45/5.0** — Medium Risk (below the 3.5 Medium/Elevated boundary). Phase-2 Timelock migration of oracle/ICL admin is executed; remaining risk is dominated by plain-EOA custody (~$151.5M across three EOAs; per-wallet max ~$93.50M) and near-total sUSDe concentration (~99.5%, USDC ~0.25%). Instant redemption pays only from the vault (~$9.09M); the custodian (~$93.50M) is an operator refill buffer, not atomic exit capacity. See Risk Score Assessment.
...
MPC team descriptions below come from Re's public protocol materials and cannot be independently verified onchain (MPC signer sets are offchain). What IS onchain-verifiable: (a) the `Governance Safe` is a **3-of-5 Safe multisig** (not MPC) at [`0x8EEc10616802Ef639ca55C98Ac856553FadeFbAd`](https://etherscan.io/address/0x8EEc10616802Ef639ca55C98Ac856553FadeFbAd) holding PROPOSER/CANCELLER on the Timelock (it no longer holds `DEFAULT_ADMIN` on reUSD, ICL, SharePriceCalculator, NAVConsumer, KYC Registry, or Deposit Token Registry); (b) the Timelock min delay is `172800` seconds = 48 hours and holds `DEFAULT_ADMIN` on SharePriceCalculator, NAVConsumer, ICL, KYC Registry, and Deposit Token Registry, plus UPGRADER_ROLE on reUSD and ICL and CUSTODIAN_MANAGER_ROLE on ICL; (c) an OpenZeppelin v5 `AccessManager` contract at [`0x3f0DA1C363e34802C6f12F9C27276dC0e6696FD8`](https://etherscan.io/address/0x3f0DA1C363e34802C6f12F9C27276dC0e6696FD8) is the `authority()` for the Instant Redemption contract and is administered by the `Access Admin` EOA below.
...
| Governance (DEFAULT_ADMIN) | [`0x8EEc10616802Ef639ca55C98Ac856553FadeFbAd`](https://etherscan.io/address/0x8EEc10616802Ef639ca55C98Ac856553FadeFbAd) | **Safe 3-of-5** (onchain-verified; not MPC) | Holds PROPOSER + CANCELLER on Timelock; does **not** hold `DEFAULT_ADMIN` on reUSD, ICL, SharePriceCalculator, NAVConsumer, KYC Registry, or Deposit Token Registry (Timelock is sole verified admin on those). UPGRADER_ROLE on reUSD and ICL held by Timelock. Also owns ShareTokenMinterBurner and ReMintBurnAdapter. | Queues Timelock actions; role administration only via 48h Timelock |
...
- **Governance improvement, phase 2 — oracle/ICL-level admin migration executed (verified onchain, Sep 23, 2026):** on Sep 2, 2026 (block 25890480, [tx `0x3daca0b0…`](https://etherscan.io/tx/0x3daca0b03e904b11b057cf6fe2d47e6d4a8a76376a563e315fa273bb59183b92)) the Governance Safe both **granted** Timelock `DEFAULT_ADMIN` where missing and **scheduled** `revokeRole(DEFAULT_ADMIN_ROLE, Safe)` on **12** targets, including **ICL** [`0x4691…3093`](https://etherscan.io/address/0x4691C475bE804Fa85f91c2D6D0aDf03114de3093), **`SharePriceCalculator`**, **`NAVConsumer`**, the **KYC Registry**, the **Deposit Token Registry**, the AUP "dust" contract [`0xE1886…3082`](https://etherscan.io/address/0xE1886BE2bA8B2496c2044a77516F63a734193082), and mirrored infrastructure for reUSDe (a second `SharePriceCalculator`/`NAVConsumer`/`DepositTokenRegistry` and an `InsuranceCapitalLayerFactory`). That batch **executed** on Sep 9, 2026 (block 25936909, [tx `0xef2b8171…346ae`](https://etherscan.io/tx/0xef2b8171d06c92f7e67d973ef562ab71fa1b73d570101a98d834303fc3a346ae)) via Timelock `executeBatch` (12× `revokeRole`). Timelock is now the sole verified `DEFAULT_ADMIN` on SharePriceCalculator, NAVConsumer, ICL, KYC Registry, and Deposit Token Registry (Safe `hasRole` = false). Because `getRoleAdmin(PRICE_SETTER_ROLE)` is still `DEFAULT_ADMIN_ROLE` and Timelock holds that role, **any `PRICE_SETTER_ROLE` grant/revoke requires the 48h Timelock queue** — closing the June price-path reversibility caveat.
...
- **Custody / asset-movement surface — ~94% of reserves now sit at plain EOAs (critical, up from ~86% in June)**:
  ...
  What's needed to drain funds onchain (per wallet):

  - If the claimed N-of-M MPC is real and Fireblocks policies are tight → compromise the policy + compromise or collude that wallet's signer quorum → **1 signed tx for that wallet** (max ~$93.50M at the custodian).
  - If Fireblocks policies are permissive → signer-quorum compromise / collusion alone → **1 signed tx for that wallet**.
  - If an insider with quorum access is malicious on one wallet → **1 signed tx for that wallet**.
  - Aggregate ~$151.5M across the three EOAs requires **three** independent compromises (or correlated offchain MPC failure — unverifiable onchain).

  This remains the single largest unmitigated custody risk in the system. The AUP provides evidence that the specific address list is in Re's MPC setup, not that unauthorized movement would be prevented by multi-party signing.
- **Onchain buffer**: Split the InstantRedemption surface carefully. The Daily Instant Redemption Vault holds ~$9.09M sUSDe (`0` USDC) — this is the **only** balance that funds atomic `redeemInstant` payouts (`vault.withdraw`). The Redemption Reserves Custodian EOA holds ~$93.50M sUSDe (`0` USDC) and is counted in `getTotalCapacity` / high-watermark math, but does **not** auto-fund redemptions without an operator refill into the vault (contract emits `RefillNeeded` when vault falls below 25% of daily limit). Do **not** treat the combined ~$102.6M as atomic instant exit capacity (up from ~$65.66M combined in June; USDC instant exits still unavailable — see Liquidity).
...
- **reUSD price**: Updated daily by a Chainlink-Functions-driven `NAVConsumer` [`0x84d4eaeb10f9e57b67622f667c6c13e22fa4b2b6`](https://etherscan.io/address/0x84d4eaeb10f9e57b67622f667c6c13e22fa4b2b6) calling `SharePriceCalculator.setSharePrice`. The NAV computation itself is **not** programmatically onchain — Chainlink Functions runs JS offchain (DON `fun-ethereum-mainnet-1`, subscription `85`) and returns a single NAV value. Onchain safeguards: Chainlink Automation triggers daily at 23:45 UTC; `NAVConsumer.maxDeviationBps = 1000` (10%) enforces a deviation guard; Hacken audited the NAV Oracle in Apr 2025 and Sherlock independently re-audited it in Jul 2026 (0 High findings). `PRICE_SETTER_ROLE` on the `SharePriceCalculator` is re-verified (Sep 7, 2026) still held **only** by `NAVConsumer` — the EOA `0x6C15B25E` remains revoked. `NAVConsumer`'s `DEFAULT_ADMIN` is the Timelock (Safe `hasRole` = false; re-verified Sep 23, 2026); `EMERGENCY_UPDATER_ROLE` remains revoked from the former EOA. **Reversibility — closed for both mint and price paths:** the Governance Safe's `DEFAULT_ADMIN_ROLE` on the reUSD **token** moved to the Timelock on Aug 10, 2026 (verified onchain), closing the gap for `MINTER_ROLE`. The matching move for `SharePriceCalculator` and `NAVConsumer` (and ICL / KYC / Deposit Token Registry) **executed Sep 9, 2026** ([tx `0xef2b8171…346ae`](https://etherscan.io/tx/0xef2b8171d06c92f7e67d973ef562ab71fa1b73d570101a98d834303fc3a346ae)) — so `PRICE_SETTER_ROLE` grants now require the 48h Timelock queue. See Token Mechanism above for the full timeline.
...
- **Instant redemption buffer (Sep 7, 2026, onchain, re-verified)**: The Daily Instant Redemption Vault at [`0x5C454f5526e41fBE917b63475CD8CA7E4631B147`](https://etherscan.io/address/0x5C454f5526e41fBE917b63475CD8CA7E4631B147) holds `0` USDC and sUSDe worth ~$9.09M (down from ~$12.71M in June) — **this is the only trustless/no-operator instant pay capacity** (`InstantRedemption` pays via `vault.withdraw` only). The `custodialWallet` (labeled "Redemption Reserves Custodian" in this report) [`0x9eA38e09F41A9DE53972a68268BA0Dcc6d2fAdf8`](https://etherscan.io/address/0x9eA38e09F41A9DE53972a68268BA0Dcc6d2fAdf8) is an EOA and holds `0` USDC and sUSDe worth ~$93.50M (up from ~$52.95M in June) — counted in `getTotalCapacity` / daily-limit watermarks, but **requires an operator refill into the vault** before it can fund `redeemInstant`. Do not treat the combined ~$102.6M as atomic exit capacity. The configured `dayPayoutToken` is still **sUSDe** (not USDC) — re-verified onchain — so instant redemptions still settle into sUSDe under current config, and (per Collateralization above) there is now almost no USDC anywhere in the reserve stack to fall back to.
...
- **Governance (re-verified onchain, Sep 7 economic snapshot; roles re-checked Sep 23, 2026)**: A **Safe 3-of-5 multisig** at [`0x8EEc10616802Ef639ca55C98Ac856553FadeFbAd`](https://etherscan.io/address/0x8EEc10616802Ef639ca55C98Ac856553FadeFbAd) (SafeProxy; still 5 owners, same addresses, threshold 3 — unchanged since June) holds `PROPOSER` + `CANCELLER` on the Timelock. It **no longer** holds `DEFAULT_ADMIN_ROLE` on the reUSD or reUSDe **tokens** (moved to the Timelock Aug 10, 2026) **or** on **ICL**, **SharePriceCalculator**, **NAVConsumer**, the **KYC Registry**, or the **Deposit Token Registry** (phase-2 Timelock migration executed Sep 9, 2026; Safe `hasRole` = false, Timelock = true — see Token Mechanism). `UPGRADER_ROLE` on reUSD and ICL remains with the Timelock Controller (unchanged). The protocol docs also describe additional MPC-controlled admin EOAs (Oracle, Redemptions, Access); those EOAs still exist onchain, unchanged, but the `N-of-M` MPC quorum remains offchain and unverifiable.
...
- **Phase-2 governance migration (executed Sep 9, 2026)**: Timelock `executeBatch` in [tx `0xef2b8171…346ae`](https://etherscan.io/tx/0xef2b8171d06c92f7e67d973ef562ab71fa1b73d570101a98d834303fc3a346ae) revoked the Governance Safe's `DEFAULT_ADMIN_ROLE` on **12** targets including ICL, `SharePriceCalculator`, `NAVConsumer`, the KYC Registry, the Deposit Token Registry, and mirrored reUSDe-side infrastructure (scheduled/granted Sep 2 in [tx `0x3daca0b0…`](https://etherscan.io/tx/0x3daca0b03e904b11b057cf6fe2d47e6d4a8a76376a563e315fa273bb59183b92)). Timelock is sole verified `DEFAULT_ADMIN` on those contracts; the `PRICE_SETTER_ROLE` reversibility gap is closed (grants require 48h Timelock).
...
- **Timelock**: 48-hour timelock on upgrades, role changes, and custodian changes routed through Timelock. This covers the token-level admin role (mint authority, since Aug 10, 2026) and the oracle/ICL-level admin role (since Sep 9, 2026).
...
- **Share Price Calculator**: [`0xd1D104a7515989ac82F1AFDa15a23650411b05B8`](https://etherscan.io/address/0xd1D104a7515989ac82F1AFDa15a23650411b05B8)
  ...
  - **Alert (High)**: Any new Timelock `CallScheduled` that would re-grant `DEFAULT_ADMIN_ROLE` on `SharePriceCalculator` / `NAVConsumer` / ICL to the Governance Safe or any EOA — phase-2 migration already executed Sep 9, 2026; reversal would reopen the price-path gap.
...
- **Timelock Controller**: [`0x69dDEa332723cF5407151aAF68B9b076557FCA93`](https://etherscan.io/address/0x69dDEa332723cF5407151aAF68B9b076557FCA93) — the 48h delay between `CallScheduled` and `CallExecuted` is the primary review window for any privileged action; the monitor must fire the moment something is queued, not when it executes.
  ...
  - **Note (resolved)**: The Sep 2, 2026 Timelock batch (schedule/grant tx [`0x3daca0b0…`](https://etherscan.io/tx/0x3daca0b03e904b11b057cf6fe2d47e6d4a8a76376a563e315fa273bb59183b92); **12** `revokeRole(DEFAULT_ADMIN_ROLE, Safe)` targets) **executed** Sep 9, 2026 in [`0xef2b8171…346ae`](https://etherscan.io/tx/0xef2b8171d06c92f7e67d973ef562ab71fa1b73d570101a98d834303fc3a346ae). Continue alerting on any new schedule that would reverse those revokes.
...
| Timelock `DEFAULT_ADMIN` holders on SharePriceCalculator / NAVConsumer / ICL (expect Timelock only) | Daily | High |
...
- **Token- and oracle/ICL-level admin control moved to the Timelock (executed)**: on Aug 10, 2026 `DEFAULT_ADMIN_ROLE` on the reUSD and reUSDe tokens moved from the 3-of-5 Governance Safe to the Timelock Controller (verified onchain via `hasRole`). On Sep 9, 2026 the matching migration for SharePriceCalculator, NAVConsumer, ICL, KYC Registry, and Deposit Token Registry also executed ([tx `0xef2b8171…346ae`](https://etherscan.io/tx/0xef2b8171d06c92f7e67d973ef562ab71fa1b73d570101a98d834303fc3a346ae)) — `MINTER_ROLE` and `PRICE_SETTER_ROLE` grants both require the 48h public Timelock queue.
...
- **Instant redemption pays only from the vault**: `InstantRedemption` settles via `vault.withdraw`; the ~$9.09M Daily Instant Redemption Vault is atomic instant capacity. The ~$93.50M Redemption Reserves Custodian is an operator-dependent refill buffer counted in `getTotalCapacity`, not equivalent exit liquidity.
...
- **$RE governance token launched, but onchain control unchanged**: the June 18, 2026 TGE and "Phase 1 governance" marketing have not yet translated into any DAO/Governor contract holding a role on the audited contracts — the same 3-of-5 Safe proposes Timelock actions, and the Timelock holds the verified `DEFAULT_ADMIN` roles on token and oracle/ICL contracts.
...
- **Oracle/setter manipulation — mint and price-path admin Timelock-gated**: `PRICE_SETTER_ROLE` on `SharePriceCalculator` remains solely `NAVConsumer` (re-verified), and Timelock holds `DEFAULT_ADMIN` on SharePriceCalculator / NAVConsumer / ICL after the Sep 9, 2026 execute — grants require 48h. Reserve assurance still rests on the offchain AUP + Grant Thornton financial audit + direct onchain audit; the Avalanche PoR feed remains a transparency mirror, not an onchain control.
- **Liquidity mismatch**: reUSD represents liquid onchain tokens partially backed by offchain reinsurance capital. Capital release is reevaluated quarterly, and programs are short-duration and cat-light (per performance memo). Trustless instant pay is vault-only (~$9.09M sUSDe, `0` USDC); the custodian (~$93.50M sUSDe) is a refill buffer, not atomic exit capacity. In a bank-run scenario, vault-funded sUSDe redemptions plus ~$27.7M in DEX liquidity would need to absorb exits for ~$252.7M in outstanding tokens until operators refill or the windowed queue handles the remainder. The underlying reserves are now almost entirely single-asset.
...
- [ ] **No audit** -- Audited by Hacken (3 public reports, including NAV Oracle), Certora with formal verification (Sep 2025), and now Sherlock (Jul 2026 NAV oracle re-audit, 0 High). 5 public smart-contract audit reports. **PASS**
...
- [ ] **Total centralization** -- MPC wallets with role separation (3-of-5 and 5-of-8). 48-hour upgrade timelock. Token- and oracle/ICL-level admin sit with the Timelock itself (not the Safe). Not a single EOA. **PASS**
...
- Onchain governance root, re-verified Sep 7, 2026 (roles re-checked Sep 23, 2026): a 3-of-5 Safe multisig (unchanged owners/threshold) holds `PROPOSER/CANCELLER` on the Timelock and **no longer** holds `DEFAULT_ADMIN` on the reUSD/reUSDe tokens (Aug 10, 2026) **or** on ICL, SharePriceCalculator, NAVConsumer, KYC Registry, or Deposit Token Registry (Sep 9, 2026). 48-hour timelock on UUPS proxy upgrades (`getMinDelay = 172800`, unchanged) and custodian changes.
...
- **Governance progress this cycle (verified onchain)**: token-level `DEFAULT_ADMIN_ROLE` (governing `MINTER_ROLE`) moved from the Safe to the Timelock on Aug 10, 2026. Matching move for `SharePriceCalculator`, `NAVConsumer`, ICL, KYC Registry, Deposit Token Registry, and mirrors **executed Sep 9, 2026** (12 revoke targets). The June-era fixes (EOA revoked from `PRICE_SETTER_ROLE`, `CUSTODIAN_MANAGER_ROLE` on Timelock, `MINTER_ROLE` reduced to 2 holders, OFT adapter/minter-burner owned by the Safe) were all re-verified unchanged.
- **Reversibility — closed for mint and price paths**: `MINTER_ROLE` and `PRICE_SETTER_ROLE` role-admin paths are Timelock-gated (48h). Safe cannot re-grant either without the public queue.
...
**Governance Score: 3.0 (improved from 3.25)** -- Both the token-level and oracle/ICL-level admin migrations to the Timelock are *executed and verified*. That closes the mint- and price-path reversibility gaps this report has carried since April and fits a 3/5 Multisig-with-timelock band on critical ops. Remaining concerns, unchanged: offchain-only MPC admin EOAs with unverifiable N-of-M quorums, no onchain DAO governance over contract control, and KYC-gated access. The largest fund-loss vector — the reserve-custody EOAs — is unchanged in kind and larger in dollar terms (captured under Collateralization, not here).

**Subcategory B: Programmability**

- reUSD price: Written onchain by a Chainlink-Functions-driven `NAVConsumer` with a 10% deviation cap (`maxDeviationBps = 1000`, re-verified unchanged). NAV computation itself runs offchain in the Chainlink DON, now independently re-audited by Sherlock (Jul 2026, 0 High). The EOA bypass remains closed — `PRICE_SETTER_ROLE` is re-verified held only by `NAVConsumer`; `EMERGENCY_UPDATER_ROLE` remains revoked from the former EOA; `DEFAULT_ADMIN` is the Timelock (Safe `hasRole` = false). `PRICE_SETTER_ROLE` grants require the 48h Timelock queue.
- Deposits: Gated by KYC Registry
- Instant redemptions: Programmatic from the vault buffer (`vault.withdraw` only; custodian is refill capacity)
- Quarterly redemptions: Admin-managed process
- Capital deployment: Entirely offchain
- Minting: `MINTER_ROLE` grants on reUSD are timelock-gated (token-level `DEFAULT_ADMIN_ROLE` moved to the Timelock, Aug 10, 2026).

**Programmability Score: 3.5 (improved from 3.75)** -- The core value-determining function (reUSD price/yield) is still set by an admin-controlled oracle, not computed programmatically onchain, but the price-path admin structure improved: Timelock now holds `DEFAULT_ADMIN` on SharePriceCalculator/NAVConsumer, so `PRICE_SETTER_ROLE` is no longer re-grantable by the Safe without delay. Capital deployment remains entirely offchain. Net: modest improvement for the closed reversibility gap; oracle remains non-programmatic.
...
**Centralization Score = (3.0 + 3.5 + 4.25) / 3 = 3.58**

**Score: 3.58/5 (improved composition)** -- Governance and Programmability both improve on the executed Timelock migration of oracle/ICL admin, while Dependencies remain worse than June on Ethena/sUSDe concentration (~99.5%). Residual concerns unchanged: offchain-only MPC admin EOAs, offchain capital deployment, and KYC gating.

#### Category 3: Funds Management (Weight: 30%)

**Subcategory A: Collateralization**

- Onchain buffer: Verifiable vault (~$9.09M atomic instant pay via `vault.withdraw`) plus custodian EOA (~$93.50M operator refill / capacity watermark) — do not equate the ~$102.6M sum with trustless exit capacity
...
**Provability Score: 3.5 (improved from 3.75)** -- Re's reserve-publication-via-Chainlink claim remains substantiated and was re-verified live this cycle. The EOA bypass of the share-price path remains closed at the `PRICE_SETTER_ROLE` level, and Timelock now holds `DEFAULT_ADMIN` on SharePriceCalculator/NAVConsumer — so `PRICE_SETTER_ROLE` grants are 48h-gated. (The new Sherlock and Grant Thornton audits are credited under Audits and Operational respectively, to avoid counting the same two facts across three categories.) Core yield calculation, reserve attestation, and performance reporting remain offchain with no independent onchain oracle to cross-check. Modest improvement for the closed price-admin reversibility gap.

**Funds Management Score = (4.5 + 3.5) / 2 = 4.0**

**Score: 4.0/5 (worse than June's 4.0 framing on custody; Provability offset)** -- Hybrid model with meaningful offchain components. The coverage ratio improved substantially, but almost entirely through sUSDe accumulation — reserves are now a near-single-asset position custodied ~94% at plain EOAs, worse than June in relative *and* absolute aggregate terms (per-wallet max ~$93.50M). That custody deterioration remains the dominant fund-loss vector. Provability improves slightly on Timelock-gated price-admin, landing the category average at **4.0**.

#### Category 4: Liquidity Risk (Weight: 15%)

- **Instant Exit**: Daily Instant Redemption Vault holds `0` USDC and sUSDe worth ~$9.09M — **atomic pay capacity** (`vault.withdraw` only). Redemption Reserves Custodian holds `0` USDC and sUSDe worth ~$93.50M — **operator refill / `getTotalCapacity` buffer**, not atomic exit (onchain, re-verified Sep 7, 2026). Configured `dayPayoutToken` is still sUSDe. Instant redemptions via [`0x8aEb9453EF22Cb38abC7a3Af9c208F65C1BfE31e`](https://etherscan.io/address/0x8aEb9453EF22Cb38abC7a3Af9c208F65C1BfE31e) settle in sUSDe under current config, subject to daily (20%) and per-wallet (10%) caps, unchanged.
...
**Score: 3.75/5 (unchanged vs June; prior 3.5 improvement reversed)** -- Onchain-verified DEX liquidity grew to ~$27.7M (~11.0% of market cap, up from ~8.2%), a real depth improvement. Instant **pay** capacity, however, is vault-only (~$9.09M, down from ~$12.71M in June); the custodian (~$93.50M) is a refill buffer counted in `getTotalCapacity`, not atomic exit — so crediting a combined ~$102.6M "instant-adjacent" buffer as exit capacity overstated Liquidity. Token-level 24h volume collapsed to ~$365K. The Aug 25 Pendle/Morpho incident is counted under Audits/Dependencies rather than double-counted here. Avalanche liquidity remains unverifiable this cycle. No USDC instant exits remain available. Net: DEX growth does not justify an improvement once vault vs custodian are separated; score held at June's **3.75**.
...
**Final Score: 3.45** (down from June's 3.51 and the Sep 7 draft's 3.50) — Medium Risk, below the 3.5 Medium/Elevated boundary. Weighted: `2.5×0.20 + 3.583333×0.30 + 4.0×0.30 + 3.75×0.15 + 2.25×0.05 = 3.45`. Governance and Programmability/Provability improve on the **executed** Sep 9 Timelock migration of oracle/ICL admin; Liquidity no longer credits custodian balances as atomic instant exit (reverts the unjustified 3.5 improvement); Funds Management lands at 4.0 with EOA custody still #1 (per-wallet max ~$93.50M). Dependencies and Collateralization remain elevated on sUSDe concentration and aggregate EOA exposure.
...
reUSD is a novel product that bridges DeFi capital with traditional reinsurance markets. Re reports (~$500M premiums written inception-to-date, ~40 carriers, ~1M policyholders, ~92% historical combined ratio — all Re-asserted, most recently updated at the June 2026 $RE TGE) and reUSD market cap ~$252.7M are meaningful and growing; the capital structure (Re Capital ~$73M **first-loss** + reUSDe **mezzanine/second-loss**) puts reUSD in the senior tranche, attaching at >115% combined ratio (~0.9% modeled loss likelihood, unchanged this cycle). The risk profile is **Medium Risk (3.45)** — down from June's 3.51 (and April's 3.55). **Current facts**: (1) token-level `DEFAULT_ADMIN_ROLE` sits with the Timelock (since Aug 10, 2026), closing the `MINTER_ROLE` reversibility gap; (2) the matching migration for the price-setting path (`SharePriceCalculator`, `NAVConsumer`, ICL, KYC Registry, Deposit Token Registry — 12 revoke targets) **executed Sep 9, 2026** ([tx `0xef2b8171…346ae`](https://etherscan.io/tx/0xef2b8171d06c92f7e67d973ef562ab71fa1b73d570101a98d834303fc3a346ae)) — `PRICE_SETTER_ROLE` grants are 48h Timelock-gated; (3) Sherlock independently re-audited the NAV oracle (Jul 2026, 0 High); (4) onchain coverage is ~67.9% all-in / ~66.7% reUSD-only, but reserves are **~99.5%/~0.25%** sUSDe/USDC — a concentration that breaches this report's June-set alert thresholds; (5) plain-EOA custody is **~94% (~$151.5M across three EOAs)**; one signature drains one wallet (per-wallet max ~$93.50M); (6) InstantRedemption pays only via `vault.withdraw` (~$9.09M vault); the custodian (~$93.50M) is a refill buffer in `getTotalCapacity`, not atomic exit capacity; (7) on Aug 25, 2026, TWAP manipulation of Pendle's PT-reUSD market triggered $36.4M in liquidations on a downstream Morpho market — Re's own contracts were unaffected; (8) the $RE governance token launched (Jun 18, 2026) but no DAO/Governor contract holds an onchain role — Safe proposes, Timelock administers. Mitigants: Safe-3-of-5 + 48-hour Timelock (`getMinDelay() = 172800`), 5 public smart-contract audits across Hacken/Certora/Sherlock, Chainlink Automation driving daily NAV updates, The Network Firm's AUP, and a Grant Thornton FY2025 financial-statement audit of the reinsurer. **EOA custody remains the single largest unmitigated risk.**
...
- **Monitor instant redemption buffer — track vault (atomic pay) and custodian (refill) separately** (currently ~$9.09M vault and ~$93.50M custodian, both 100% sUSDe); alert on `RefillNeeded` and vault drawdowns
...
- **Monitor Timelock `DEFAULT_ADMIN` holders** on SharePriceCalculator / NAVConsumer / ICL (expect Timelock only; phase-2 migration already executed Sep 9, 2026)
...
- **Governance-based**: Reassess if Timelock `DEFAULT_ADMIN` on SharePriceCalculator / NAVConsumer / ICL is re-granted to the Safe or an EOA, when any DAO/Governor contract is granted an onchain role, or after any further role changes or fund redeployments
...
│  │  NO DEFAULT_ADMIN on reUSD/reUSDe tokens    │                    │
│  │  (Aug 10, 2026) or ICL/SharePriceCalculator │                    │
│  │  /NAVConsumer/KYC/DepositTokenRegistry      │                    │
│  │  (executed Sep 9, 2026)                     │                    │
│  └─────────────────────────────────────────────┘                    │
│                                                                     │
│  ┌─────────────────────────────────────────────┐                    │
...
│  │  0x69dDEa..57FCA93                          │                    │
│  │  DEFAULT_ADMIN on token + oracle/ICL path;  │                    │
...
Fund Flow:
  ...
  (EOA PRICE_SETTER_ROLE revoked Jun 2026, re-verified still revoked; only NAVConsumer writes. SharePriceCalculator/NAVConsumer/ICL DEFAULT_ADMIN is Timelock since Sep 9, 2026 execute — PRICE_SETTER grants require 48h Timelock.)
...
Trust Boundaries:
  ...
  ⚠ Share price path: PRICE_SETTER_ROLE still only NAVConsumer (EOA bypass remains closed); SharePriceCalculator DEFAULT_ADMIN is Timelock (executed Sep 9, 2026) — grants 48h-gated
  ...
  ⚠ NEW: ~94% of onchain reserves ($151.5M) sit at three plain EOAs (per-wallet max ~$93.50M), up from ~86% in June
  ⚠ InstantRedemption: vault.withdraw only (~$9.09M atomic); custodian (~$93.50M) is refill/capacity, not atomic exit
```
...
| Date | Score | Notes |
| --- | --- | --- |
| [April 17, 2026](https://github.com/yearn/risk-score/pull/84) | 3.6 | Initial assessment |
| [June 21, 2026](https://github.com/yearn/risk-score/pull/262) | 3.51 | Reassessment: governance improvements (EOA PRICE_SETTER revoked; Timelock CUSTODIAN_MANAGER/UPGRADER); score held near Elevated floor after coverage/attachment review |
| [September 7, 2026](https://github.com/yearn/risk-score/pull/446) | 3.45 | Sep 7 economic snapshot; Sep 23 verification update: phase-2 Timelock migration executed Sep 9 (12 revoke targets); InstantRedemption vault-vs-custodian split; EOA loss scenario per-wallet max ~$93.50M; critical gates unchecked; scores → Final 3.45 |
