...
- Live-chain simulation (`eth_call`, no transaction sent) at block [25,990,834](https://etherscan.io/block/25990834): the admin has no code and signed a [real mainnet transaction](https://etherscan.io/tx/0xec68b97d5ae2f8bf86e4968f151f4db0a7c1c809a843f77eebb6ee6aaa06fe65), so it is a private-key EOA rather than a contract. The proxy bytecode contains all transparent-proxy admin functions (stored as negated selector constants). `admin()` called by the EOA returns the EOA, and `upgradeTo` from the EOA succeeds while the same call from a random address reverts. `Vault.execute(uniBTC, mint(0x…dEaD, 100,000 uniBTC), 0)` succeeds with the router as sender and reverts with `AccessControl` from a random address.
- A [Foundry fork test](https://github.com/yearn/risk-score/blob/da85db8d6c2794ac1042034a2215f877b744a377/reports/data/bedrock-unibtc-legacy-router-test) at block 25,981,132 reproduces this end to end and passes: the admin EOA upgrades the router to a minimal malicious implementation, which mints 100,000 uniBTC to a sink through `Vault.execute`; total supply and the sink balance each rise by exactly that amount. Note that `hasRole(DEFAULT_ADMIN_ROLE, 0x3eea…)` returns `false` by design and does **not** contradict this: on a `TransparentUpgradeableProxy` the upgrade right is the EIP-1967 admin slot, not an AccessControl role, so an Etherscan "Read as Proxy" `hasRole` check reads the wrong permission. The test confirms the router's `DEFAULT_ADMIN_ROLE` holder cannot upgrade, while the admin-slot EOA can.
...
- **Secondary exit:** [CoinGecko](https://www.coingecko.com/en/coins/universal-btc) reports only **$4,587** in 24h volume across all venues; the largest ticker had about $4,030 and the Ethereum Uniswap V3 uniBTC/WBTC ticker about $200 of that volume. The indicative price is **0.992417 BTC**. Thin trading makes the displayed near-par price weak evidence of executable depth.
- **Onchain depth:** Ethereum liquidity sits almost entirely in the Uniswap V3 0.05% pool [`0x3a32…3FDa`](https://etherscan.io/address/0x3a32F5040Bc4d8417e78E236eb2C48c90e003FDa), which held **33.81879472 uniBTC and 11.02957078 WBTC** at the snapshot. The 0.01%, 0.3% and 1% pools and the Uniswap V2 pair held dust (Curve and Balancer were not checked). QuoterV2 returned 0.99273912 WBTC for 1 uniBTC and 11.02672935 WBTC for both 13 and 65 uniBTC, so executable exit size is capped by that ~11 WBTC inventory.
- **Indicative exits:** [CoW Protocol quote API](https://api.cow.fi/mainnet/api/v1/quote) responses into WBTC returned **0.99253745 WBTC for 1 uniBTC** (0.75% below parity), **11.02803585 WBTC for 13 uniBTC (~$1M)** (15.17% below) and **11.02805981 WBTC for 65 uniBTC (~$5M)** (83.03% below). Quote IDs are in the snapshot evidence. These were unsigned, expiring indications, not executed swaps or guaranteed future fills. Both larger quotes cap at the same ~11 WBTC Uniswap pool inventory; large exits remain severely constrained.
...
5. **Reserves are mostly native BTC.** 78.62% of dashboard reserves sit in 44 published Bitcoin addresses rather than wrapped or bridged tokens. These addresses are issuer-declared; custody, signers and Babylon encumbrances are not independently verified.
