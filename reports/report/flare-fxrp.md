...
- **The mesh topology qualifies the "bounded by escrow" framing.** Because Ethereum peers five sibling OFTs directly (Monad, HyperEVM, Base, BNB, Katana) and not only the escrow-holding Flare adapter, Ethereum FXRP can be minted by a message that never touches the Flare escrow at all. Anyone who can mint on *any* peered chain — for example by upgrading that chain's OFT proxy — can bridge the result into Ethereum and receive genuine, fungible Ethereum FXRP that no escrow backs. The correct blast-radius statement is therefore: bounded away from canonical FXRP and XRP, but **not** bounded by the escrow on the Ethereum side.
