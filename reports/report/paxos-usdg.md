...
- **Total Supply (Ethereum):** ~452,279,600 USDG (~$452M) [onchain](https://etherscan.io/token/0xe343167631d89B6Ffc58B88d6b7fB0228795491D). DeFiLlama reports ~$415M as *circulating on Ethereum* — the difference is ~$41M minted on Ethereum and bridged out to Ink and Hyperliquid
- **Total Supply (All Chains):** ~$3.29B [DeFiLlama](https://stablecoins.llama.fi/stablecoin/286)
- **Market Cap:** ~$3.29B
- **30-Day Supply Change:** +$435M (+15.3%) — from $2,850.8M (June 30) to $3,286.1M (July 30) [DeFiLlama](https://stablecoins.llama.fi/stablecoincharts/all?stablecoin=286)
...
| Hyperliquid L1 | `USDG0` HIP-1 spot token (index 352) | $1.8M | <0.1% | none — not a wrapper peer |
...
- **Every configured inbound route requires the same 3-of-3 DVN quorum.** Read from `EndpointV2.getConfig(wrapper, receiveLib, srcEid, 2)`: required DVNs are LayerZero Labs [`0x589d…236b`](https://etherscan.io/address/0x589dedbd617e0cbcb916a9223f4d1300c294236b), Canary [`0xa4fe…c2cd`](https://etherscan.io/address/0xa4fe5a5b9a846458a70cd0748228aed3bf65c2cd), and Paxos [`0xb0b2…2daf`](https://etherscan.io/address/0xb0b2ef168f52f6d1e42f461e11117295ef992daf) on all of Robinhood Chain (40 block confirmations), X Layer (168), and Solana (32), with `optionalDVNCount = 0`. Adding Robinhood Chain did not introduce a weaker route — but note that Paxos itself operates one of the three required DVNs, so the quorum is 3-of-3 across only two independent parties.
- **Hyperliquid L1 is not reachable from the Paxos wrapper.** The balance DeFiLlama attributes to LayerZero is a HIP-1 spot token named `USDG` with `fullName: USDG0`, `tokenId: 0xae87b5246dd9f377b14bbadcf3c72131`, `isCanonical: false`, and no linked EVM contract (Hyperliquid `info` API, `{"type":"spotMeta"}`). The operator of that USDG0 route is **unverified** — it is not a Paxos-configured peer, so it is a third-party wrapper rather than a Paxos-controlled path. Checks already performed (so a future reassessment need not repeat them): the wrapper's full `PeerSet` history has no Hyperliquid eid; no USDG contract exists at the canonical address on HyperEVM; LayerZero's public OFT registry returns no USDG entry; Hyperliquid's `tokenDetails` and Hypurrscan return no genesis/deployer record for this token id; and Etherscan's holder list (which would reveal an Ethereum escrow of ~1.8M USDG) is a Pro-tier endpoint. Identifying the operator needs either a Pro holder-list query or a log scan of USDG `Transfer` events for a ~1.8M-balance contract.
