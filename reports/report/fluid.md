...
- **Final Score: 2.3/5.0** (up from 1.4; weighted subtotal 1.94 → 2.34 as SUSDAI crossed the 30% trigger, the issuer was identified as USD.AI, the existing REUSD risk assessment [score 3.51] was cross-referenced, and ~37% of the collateral book was found to rely on offchain/admin-priced marks; -0.5 TVL modifier removed — protocol had a material bad-debt incident in Mar 2026)
...
- **However, ~37% of the collateral book is priced by offchain/admin-controlled inputs that cannot be independently verified onchain**, so whether the fTokens are actually fully backed is *not* purely onchain-provable:
  ...
  - **sUSDAI** ($199.2M): its value derives from USD.AI's offchain AI-hardware loan book; there is no onchain proof of the underlying collateral, and its market price is set on a venue Fluid itself dominates.
...
**Score: 2.875/5** — (3.75 + 2.0) / 2 = 2.875 (was 1.875 on Jul 6). Collateralization 3.25 → 3.75 for the collateral-quality, illiquidity, and realized-loss factors above; Provability 1.0 → 2.0 because ~37% of the collateral is priced offchain/admin-side and forming bad debt is not onchain-observable.
...
**Final Score: 2.3375** → rounded to **2.3** (was 1.4 in May 2026; was 1.1 in Feb 2026). Underlying weighted subtotal moved 1.94 → 2.34 driven by: (a) SUSDAI concentration crossing and holding above the 30% trigger and sUSDAI issuer identified as USD.AI — Funds Mgmt § A (Collateralization) 2.75 → 3.75 (illiquid single-asset concentration worse than reUSD, which we score 4.25; realized Mar-2026 loss at smaller exposure); (b) Funds Mgmt § B (Provability) 1.0 → 2.0 (~37% of the collateral book is priced offchain/admin-side — reUSD `setSharePrice`, USD.AI loan-book NAV — so forming bad debt is not onchain-observable); (c) Category 2 § C (Dependencies) 2.5 → 3.5 (single novel issuer USD.AI + REUSD cross-reference to the 3.51-rated assessment). The -0.5 TVL modifier is not applied (the protocol had a material Mar-2026 bad-debt incident, disqualifying a discretionary reduction).
...
**Final Risk Tier: LOW RISK** (moved from Minimal Risk; final score 1.4 → 2.3; driven by SUSDAI concentration above the 30% trigger, sUSDAI issuer identified as USD.AI, ~37% of the collateral book found to rely on offchain/admin-priced marks, REUSD 3.51 cross-reference, and non-application of the -0.5 TVL modifier due to the Mar 2026 incident)
...
Factors driving the score movement from 1.4 to 2.3: (1) the **30% concentration trigger firing** — SUSDAI is now 31.2% of cross-chain TVL and still rising; (2) the **identification of sUSDAI's issuer as USD.AI**, which surfaced that Fluid holds ~two-thirds of all sUSDAI supply (~$199M) with extreme on-chain illiquidity and that sUSDAI is not a stable-value instrument; (3) **cross-referencing the existing REUSD risk assessment** (score 3.51/5.0, Medium Risk), which shows $38.8M (6.1% of Fluid TVL, ~26.7% of reUSD supply) is in an asset with ~50% offchain backing, admin-written share prices, EOA reserve custody, and no bug bounty; and (4) the finding that **~37% of the collateral book is priced by offchain/admin-controlled marks** (reUSD `setSharePrice`, USD.AI's offchain loan-book NAV), so forming bad debt is not onchain-observable in advance. Combined SUSDAI + REUSD: ~37% of TVL in yield-bearing wrappers from two high-risk issuers, one of which we independently rate Medium Risk. **The -0.5 TVL modifier is not applied** — a protocol with a material bad-debt event in the past ~4 months should not receive a discretionary score reduction. Funds Mgmt § A (Collateralization) moves 2.75 → 3.75, § B (Provability) 1.0 → 2.0, and Category 2 § C (Dependencies) 2.5 → 3.5 (weighted subtotal 1.94 → 2.34). At one-decimal rounding the final score moves to 2.3, shifting from Minimal Risk to **Low Risk** tier — now sitting closer to the Medium boundary, appropriate for a protocol carrying a larger, more-illiquid single-asset concentration than the one that produced its March 2026 bad-debt event.
...
| July 14, 2026 | 2.3 | Reassessment: lending TVL $639.0M (−26.8% off May peak, stabilizing); SUSDAI above 30% trigger at 31.2% and issuer identified as USD.AI (Fluid holds ~2/3 of supply, extreme illiquidity); reUSD cross-referenced to its 3.51 Medium-Risk report; ~37% of collateral priced offchain/admin-side. Collateralization 2.75 → 3.75, Provability 1.0 → 2.0, Dependencies 2.5 → 3.5; GovernorBravo corrected to 127/135 executed. Minimal → Low Risk |
