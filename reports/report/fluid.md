...
- **Assessment Date:** February 12, 2026 (Updated: July 14, 2026; prior reassessments Apr 27, May 24, Jul 6, 2026)
...
- **Final Score: 2.4/5.0** (up from 1.4; weighted subtotal 1.94 → 2.375 as SUSDAI crossed the 30% trigger, the issuer was identified as USD.AI, the USDai vault oracle was found to be a depeg-blind hardcoded 1:1 peg at ~94% CF, the existing REUSD risk assessment [score 3.51] was cross-referenced, and ~37% of the collateral book cannot be onchain-verified against true value; -0.5 TVL modifier removed — protocol had a material bad-debt incident in Mar 2026)

## Reassessment Summary (July 2026)

Onchain refresh at block `25529610` (July 14, 2026). Governance, proxy implementations, and rate models are all unchanged since May 24. The material changes are the sustained TVL decline off the May peak (now stabilizing) and SUSDAI concentration holding above the 30% trigger. **sUSDAI is issued by USD.AI (usd.ai)**, a synthetic-dollar protocol backed by AI hardware loans. This surfaces additional risk factors including Fluid holding roughly two-thirds of total sUSDAI supply and extreme on-chain illiquidity.

**Material change #1 — SUSDAI concentration above the 30% trigger:** Top supply asset SUSDAI is **31.2%** of cross-chain lending TVL ($199.2M), up from 30.6% on Jul 6 and 28.3% on May 23 — the explicit reassessment trigger set in May 2026 has fired and the share is still rising. Per-chain it is **63.9% of Arbitrum and 75.3% of Plasma supply** (down from May peaks of 75.3% / 80.4% as diverse TVL exited those chains faster than SUSDAI did, though the cross-chain share keeps climbing). Same yield-bearing-stable-wrapper pattern as the wstUSR exposure that produced the Mar 2026 bad-debt event. **Issuer is USD.AI (usd.ai).** **Additional concentration risks:** Fluid holds ~$199M of sUSDAI — roughly two-thirds of its ~$300M supply — and sUSDAI 24h on-chain volume is only ~$681K (~0.2% of market cap), making exit nearly impossible without severe slippage. sUSDAI has historically traded between $0.796 and $1.19, confirming it is not a stable-value token. The USDai vault oracle is also a **depeg-blind hardcoded 1:1 peg** (`PegOracleL2`, verified onchain) at ~94% CF, so a USDai depeg would not trigger liquidations — a structural bad-debt path on the dominant collateral. Funds Mgmt § A (Collateralization) moves 2.75 → 4.0 and § B (Provability) 1.0 → 2.0; Category 2 § C (Dependencies) moves 2.5 → 3.5; weighted subtotal 1.94 → 2.375. The -0.5 TVL modifier is removed (protocol had a material incident): final score **2.4** (up from 1.4; moves from Minimal Risk to Low Risk tier).
...
### fToken On-Chain State (Ethereum Mainnet, verified July 14, 2026 at block `25529610`)

| fToken | Total Assets (Jul 14 2026) | Total Assets (Jul 6 2026) | Exchange Rate (Jul 14) | Exchange Rate (Jul 6) | Δ Rate (Jul 6→Jul 14) |
|--------|---------------------------|---------------------------|-----------------------|------------------------|------------------------|
| fUSDC | 134.4M USDC | 144.0M USDC | 1.206 | 1.204 | +0.16% |
| fUSDT | 130.5M USDT | 123.9M USDT | 1.196 | 1.189 | +0.61% |
| fGHO | 15.45M GHO | 14.2M GHO | 1.117 | 1.116 | +0.10% |
| fwstETH | 359.8 wstETH | 358.0 wstETH | 1.038 | 1.038 | +0.00% |
| fWETH | 1,599.5 WETH | 1,548.7 WETH | 1.078 | 1.078 | +0.00% |
| fUSDtb | 2.22M USDtb | 2.47M USDtb | 1.025 | 1.022 | +0.33% |
| fsUSDS | 5,015 sUSDS | 5,015 sUSDS | 1.002 | 1.002 | +0.00% |

All exchange rates **continue to increase monotonically** through every checkpoint (Feb 2026 → Apr 27 → May 11 → May 24 → Jul 6 → Jul 14, verified onchain). This is the key safety property of the ERC4626 fToken design and confirms that fToken holders have not lost any principal value on Ethereum at any point.
...
### Core Infrastructure (Dependency for Lending) — Verified Onchain July 14, 2026 at block `25529610`
...
- **Liquidity Layer (proxy)**: [`0x52Aa899454998Be5b000Ad077a46Bbe360F4e497`](https://etherscan.io/address/0x52Aa899454998Be5b000Ad077a46Bbe360F4e497) — Central contract holding all funds. Upgradeable proxy (Instadapp **Infinite Proxy** — uses a Fluid-custom slot layout, *not* standard EIP-1967). The dispatch logic lives in module slots; the impl-slot holds a "dummy" implementation purely so block explorers can detect the proxy.
  ...
  - **Dummy-implementation slot** (Fluid custom: `0x360894a13ba1a3210667c828492db98dca3e2076cc3735a920a3ca505d382bbc` — defined in [`infiniteProxy/proxy.sol`](https://github.com/Instadapp/fluid-contracts-public/blob/main/contracts/infiniteProxy/proxy.sol)): changed once between assessments. At block `24436972` (Feb 12, 2026) the slot held [`0xa57d7cEF617271F4cEa4F665D33eBcFCbA4929f6`](https://etherscan.io/address/0xa57d7cEF617271F4cEa4F665D33eBcFCbA4929f6); at block `24779519` (Mar 31, 2026) it was upgraded to the current [`0xcc331DaF69752Bece3Dc98DBc63EacD5092266a2`](https://etherscan.io/address/0xcc331DaF69752Bece3Dc98DBc63EacD5092266a2). Both impls are verified as `FluidLiquidityDummyImpl` (compiler `0.8.21` → `0.8.29` respectively). **Verified unchanged at block `25529610` (July 14, 2026).** The standard EIP-1967 impl slot is **empty** — Fluid does not use it.
    ...
    - Scheduled by **governance proposal #126** — verified via the `ProposalExecuted(126)` event emitted by GovernorBravo within the upgrade tx (block 24779519, Mar 31 2026). **No 2026-dated audit covering the new module version is published** at [audits-and-security.html](https://docs.fluid.instadapp.io/audits-and-security.html) (rechecked July 14 2026); the most recent posted audits are MixBytes (Sep–Dec 2025) and StateMind (Sep–Oct 2025), both on the Liquidity Layer. Treat the `0xcc33…` impl as in-scope of those 2025 reviews unless a separate post-upgrade audit is later published.
- **LendingFactory**: [`0x54B91A0D94cb471F37f949c60F7Fa7935b551D03`](https://etherscan.io/address/0x54B91A0D94cb471F37f949c60F7Fa7935b551D03) — `owner() = 0x2386DC45...` (Timelock) ✓ (rechecked block `25529610`)
- **Timelock**: [`0x2386DC45AdDed673317eF068992F19421B481F4c`](https://etherscan.io/address/0x2386DC45AdDed673317eF068992F19421B481F4c) — `delay() = 86400` (1 day) ✓; `admin() = 0x0204Cd03...` (GovernorBravo) ✓ (rechecked block `25529610`)
- **GovernorBravo**: [`0x0204Cd037B2ec03605CFdFe482D8e257C765fA1B`](https://etherscan.io/address/0x0204Cd037B2ec03605CFdFe482D8e257C765fA1B) — `proposalCount() = 135` (was 131 on May 24 2026, 128 on May 11, 117 in Feb 2026). Re-verified onchain at block `25529610` by iterating `state(uint256)` for all 135 proposals: **127 are in state `Executed`** (state==7); the other 8 are terminal non-executed states — 3 Canceled (#44, #50, #72), 3 Defeated (#87, #93, #121), 2 Expired (#117, #124). The 4 proposals created since May 24 (#132–#135) and the prior #129–#131 have all executed.
  - **Proposal 128 executed on May 5, 2026** at 05:43 UTC ([tx](https://etherscan.io/tx/0x6e0273ae285520126687f852efd623a0c84b351a6873850553da331d0c2430f5), block `25007782`). Although the proposal text said it would move the V2 USDC/USDT kinks from 85%/93% to 90%/95%, the execution receipt emitted `LogUpdateRateDataV2s` only for ETH (`rateAtUtilizationMax` 100% -> 10%). Re-verified current onchain values at block `25529610`: USDC and USDT remain at 85%/93% with 5.40%/7.50% kink rates.
  - quorumVotes: 4,000,000 FLUID ✓; proposalThreshold: 1,000,000 FLUID ✓; votingDelay: 7,200 blocks ✓; votingPeriod: 14,400 blocks ✓ (all rechecked at block `25529610`)
- **Avocado Multisig (Timelock guardian)**: [`0x4F6F977aCDD1177DCD81aB83074855EcB9C2D49e`](https://etherscan.io/address/0x4F6F977aCDD1177DCD81aB83074855EcB9C2D49e) — Custom contract (not a Gnosis Safe; standard `getThreshold()` / `getOwners()` revert). Signers exposed via `signers()(address[])` and threshold via `requiredSigners()(uint256)`. Re-verified onchain July 14 2026 (block `25529610`): **7-of-14 multisig** (unchanged from May 24 2026; was 8-of-16 on Apr 27 2026), `owner() = 0xC7810aA3b0c6A2778EEcC114B93d59B2E9Da9E05` (also one of the 14 signers).
  - Signers (14, unchanged from May 24): `0x1d895E5C…`, `0x33581f26…`, `0x4604E3bF…`, `0x5612C18E…`, `0x7284a845…`, `0x88bB9B99…`, `0x97399C93…`, `0xA32e5237…`, `0xa385B298…`, `0xa7615CD3…`, `0xC0c72156…`, `0xc1490E04…`, `0xC7810aA3…`, `0xD33D3fcE…`.
- **FLUID Token**: [`0x6f40d4A6237C257fff2dB00FA0510DeEECd303eb`](https://etherscan.io/address/0x6f40d4A6237C257fff2dB00FA0510DeEECd303eb) — Governance token. `totalSupply() = 100,000,000 FLUID` ✓ (verified block `25529610`).
...
Per-chain utilization figures below are from the May 6 2026 LiquidityResolver refresh (carried forward — the July 14 2026 reassessment did not re-iterate all six chains' resolvers). Cross-chain supply totals **were** refreshed to July 14 2026 from DeFiLlama; per-chain supply USD is current as of that snapshot.

| Chain | Per-Chain Supply (Jul 14 2026) | Per-Chain Supply (May 23 2026) | Δ | Listed Tokens | USD-Weighted Util (May 6) | Highest Borrowed-Token Util (May 6) |
|-------|------------------------------|-------------------------------|------|---------------|---------------------------|--------------------------------------|
| Ethereum | $426.1M | $531.2M | -19.8% | 38 | ~52.5% | GHO 86.7%, ETH 84.9%, USDT 77.6%, USDC 76.1% |
| Arbitrum | $111.2M | $185.4M | -40.0% | 22 | ~45.2% | USDC 90.3%, USDT0 88.6%, GHO 86.3%, ETH 81.1% |
| Base | $18.5M | $24.8M | -25.4% | 23 | ~40.4% | USDC 86.5%, GHO 84.0%, ETH 70.5% |
| Polygon | $3.4M | $4.5M | -24.4% | 13 | ~31.7% | USDT0 92.1%, USDC 87.3%, WETH 80.6% |
| Plasma | $79.9M | $126.5M | -36.8% | 16 | ~44.3% | USDT0 90.9%, USDe 58.5%, GHO 46.7% |
| BNB | n/a (no lending TVL reported in DeFiLlama snapshot) | n/a | — | 10 | ~43.3% | USDT 96.7%, USDC 90.1%, WETH 84.9% |

Combined lending TVL: $1.28B (Feb 2026) → $750.8M (Apr 27 trough, -41%) → $872.5M (May 23 peak, +16% from trough) → $631.9M (July 6) → **$639.0M (July 14, -26.8% from May 23)**. The May–June decline was broad-based across all chains — vs May 23, Ethereum -19.8%, Arbitrum -40.0%, Plasma -36.8%, Base -25.4%, Polygon -24.4% — then flattened in early July. No chain escaped the drawdown.
...
### Interest Rate Model — Re-verified Onchain July 14, 2026 at block `25529610`
...
### Lending TVL by Asset Type — Refreshed July 14, 2026

Source: DeFiLlama `fluid-lending` `chainTvls.Ethereum.tokensInUsd` snapshot dated 2026-07-14 (Ethereum-only, total $426.1M of supply collateral routed through the Liquidity Layer; this includes Vault-collateral deposits, not only fToken supply). Categories computed directly from the token-level breakdown.
...
Yield-bearing stablecoin wrappers are **~25% of Ethereum supply**, now the second-largest category behind ETH/LSTs and larger than plain stablecoins. SUSDAI is present on Ethereum at **$64.6M (15.2% of Eth supply)**, up from ~$52M on Jul 6 and near-zero in May — its expansion onto Ethereum adds to the cross-chain concentration concern.

### Top Supply Assets (Cross-Chain) — Refreshed July 14, 2026

Source: DeFiLlama `fluid-lending` `tokensInUsd` snapshot dated 2026-07-14 (cross-chain, total $639.0M).

| Rank | Token | Supply TVL | % of Total | Δ vs Jul 6 share |
|------|-------|-----------|------------|---------------------|
| 1 | **SUSDAI** | **$199.2M** | **31.2%** | **+0.6 pp** |
| 2 | **WSTETH** | $133.2M | **20.8%** | +0.1 pp |
| 3 | WETH | $42.1M | 6.6% | +0.3 pp |
| 4 | **REUSD** | $38.8M | **6.1%** | -1.8 pp |
| 5 | USDT | $27.4M | 4.3% | +0.4 pp |
| 6 | PST | $26.5M | **4.2%** | +1.5 pp |
| 7 | USDT0 | $26.0M | 4.1% | +0.9 pp |
| 8 | USDC | $25.8M | 4.0% | -0.4 pp |
| 9 | WBTC | $24.9M | 3.9% | -0.3 pp |
| 10 | CBBTC | $23.5M | 3.7% | +0.1 pp |
| 11 | WEETH | $18.1M | 2.8% | -0.4 pp |
| 12 | OSETH | $11.0M | 1.7% | — |
| 13 | ETH | $10.3M | 1.6% | -0.1 pp |
...
### Governance — Re-verified onchain July 14, 2026 at block `25529610`
...
**Score: 2.0/5** (unchanged from Apr 27 and May 24). Strong audit coverage and bounty unchanged. History grew from ~2.18 → ~2.40 years but is still not "incident-free": one bad-debt event covered via off-balance-sheet capital, plus one precautionary freeze.
...
**Subcategory C: Dependencies — 3.5** (was 2.5)
...
- External collateral-issuer dependency remains a first-order risk (Mar/Apr 2026 events). The SUSDAI concentration at 31.2% (cross-chain) and 64–75% (per-chain on Arbitrum/Plasma) makes a single novel issuer (**USD.AI**) the dominant external dependency. Fluid holds ~$199M of sUSDAI — roughly two-thirds of its ~$300M supply — with 24h on-chain volume of only ~$681K, making any exit from the position effectively impossible without cascading price impact. USD.AI has no standalone Yearn risk assessment and is newer and less-scrutinized than the issuers behind Fluid's blue-chip collateral. **Additionally, REUSD ($38.8M, 6.1% cross-chain) has a standalone risk score of 3.51/5.0 (Medium Risk)** per [`reports/report/re-reusd.md`](re-reusd.md): ~50% offchain backing via reinsurance trusts, share price written daily by admin-controlled Chainlink Functions (`setSharePrice`), ~86% of onchain reserves held at plain EOAs, no bug bounty, and only 42% branch test coverage. Fluid holds ~26.7% of all reUSD supply.

**Score: 2.33/5** (was 2.0 in May). (2.0 + 1.5 + 3.5) / 3 = 2.33. Dependencies subcategory raised 2.5 → 3.5: (a) sUSDAI issuer identified as USD.AI — a single novel synthetic-dollar issuer on which 31.2% of lending TVL depends, where Fluid holds ~two-thirds of total supply and no standalone assessment exists (2.5 → 3.0); plus (b) **REUSD cross-reference** to the existing 3.51-rated (Medium Risk) assessment at `reports/report/re-reusd.md`, an asset with ~50% offchain backing, admin-written share prices, EOA-based reserve custody, and no bug bounty (3.0 → 3.5). With sUSDAI (31.2%) and REUSD (6.1%), ~37% of Fluid's lending TVL is in yield-bearing wrappers from two high-risk issuers (USD.AI and Re Protocol), one of which we independently rate Medium Risk.

#### Category 3: Funds Management (Weight: 30%)

**Subcategory A: Collateralization — 4.0** (was 3.75 pre-oracle-review, 3.25 on Jul 6, 2.75 in May, 2.5 in Apr)
- All lending is over-collateralized per-position via Vault Protocol with onchain tick-based liquidations (unchanged) — the *backing* axis is strong.
...
- **Un-liquidateable concentration undercuts the over-collateralization defense.** Because Fluid *is* the sUSDAI market (~100% of on-chain trading volume, ~two-thirds of supply), 31.2% of the collateral book cannot be liquidated into external depth without cascading price impact — the same failure mode that turned the March 2026 wstUSR depeg into realized bad debt, now at a *larger* single-asset exposure than the 18.9% that caused that incident.
...
- **Oracle cannot reflect a depeg (verified onchain, Jul 14).** Fluid prices its USDai vaults (the base of the sUSDAI exposure; Arbitrum/Base T1 vaults, e.g. `VaultT1_USDai_USDC`) with [`PegOracleL2`](https://arbiscan.io/address/0xdf79ee3ab9ae7631a9b109d7345136274119fc72) — a **hardcoded 1:1 peg**: `_getPegExchangeRate()` returns a constant (`1e15`) with ERC4626 feed = `address(0)`, and `getExchangeRateOperate()` == `getExchangeRateLiquidate()` with no market or Chainlink input. At **~94% collateral factor** (CF 9400 / liq threshold 9500, verified via VaultResolver), if USDai falls below ~$0.94 the position is real-underwater but the oracle still reports $1 — so **liquidations never fire and the shortfall becomes bad debt**. This is the exact wstUSR March-2026 shape, on an asset that has already traded to $0.796 (−20%). (Staked sUSDai DEX vaults on Arbitrum/Plasma use reserve-based oracles on Fluid-dominated venues — a related circularity risk.)
- **Calibration:** 4.0 reflects that the *dominant* collateral is both illiquid/concentrated **and** priced by a depeg-blind hardcoded peg at ~94% CF — a concrete structural bad-debt path, not merely illiquidity. It sits at reUSD-adjacent territory (reUSD alone scores Collateralization 4.25) but below structurally-undercollateralized credit books (3jane/infinifi 4.5), since Fluid's per-position over-collateralization and (for most other collateral) working liquidations are real. The realized Mar-2026 loss at a *smaller* concentration, a #1 asset worse than reUSD, and a peg oracle that cannot express its depeg together lift it from the "mixed quality" 3.0–3.25 band to 4.0.

**Subcategory B: Provability — 2.0** (was 1.0)
...
- **However, ~37% of the collateral book is priced by offchain/admin-controlled inputs that cannot be independently verified onchain**, so whether the fTokens are actually fully backed is *not* purely onchain-provable:
  ...
  - **sUSDAI / USDai** ($199.2M): the underlying backing is USD.AI's **offchain AI-hardware loan book** — not onchain-verifiable — and Fluid's vault oracle does not even read it: the USDai vaults use a **hardcoded 1:1 peg** (see Collateralization above), so the system cannot onchain-prove the collateral is worth what it credits, and a real depeg is invisible to it.
...
- Net: a step down from a purely onchain-provable base (rubric row 2, "mostly onchain, some offchain / single reliable source"), consistent with aave-sGHO's 1.5 for a milder manual-verification gap — Fluid's offchain-marked collateral share is larger.

**Score: 3.0/5** — (4.0 + 2.0) / 2 = 3.0 (was 1.875 on Jul 6). Collateralization 3.25 → 4.0 for the collateral-quality, illiquidity, realized-loss, and depeg-blind-peg-oracle factors above; Provability 1.0 → 2.0 because for ~37% of the collateral the system cannot onchain-prove the backing is worth what it credits (reUSD admin-NAV; USDai hardcoded peg over an unverifiable offchain loan book).
...
**Score: 2.0/5** (unchanged). Withdrawal mechanism robustness validated by Mar/Apr 2026 stress events and the uneventful May–Jul 2026 drawdown. Concentration risk is rising but the score already reflected the structural pattern — the magnitude change is captured in Funds Mgmt § A rather than double-counted here.
...
| Centralization & Control | 2.33 | 30% | 0.700 |
| Funds Management | 3.0 | 30% | 0.900 |
| Liquidity Risk | 2.0  | 15% | 0.300 |
| Operational Risk | 1.5  | 5% | 0.075 |
| **Subtotal** | | | **2.375** |

**Final Score: 2.375** → rounded to **2.4** (was 1.4 in May 2026; was 1.1 in Feb 2026). Underlying weighted subtotal moved 1.94 → 2.375 driven by: (a) SUSDAI concentration crossing and holding above the 30% trigger, sUSDAI issuer identified as USD.AI, and the **USDai vault oracle found to be a depeg-blind hardcoded 1:1 peg at ~94% CF** — Funds Mgmt § A (Collateralization) 2.75 → 4.0 (illiquid single-asset concentration worse than reUSD, which we score 4.25; realized Mar-2026 loss at smaller exposure; peg oracle that cannot express a depeg); (b) Funds Mgmt § B (Provability) 1.0 → 2.0 (for ~37% of collateral the backing is not onchain-provable — reUSD `setSharePrice` admin-NAV, USDai hardcoded peg over an unverifiable offchain loan book); (c) Category 2 § C (Dependencies) 2.5 → 3.5 (single novel issuer USD.AI + REUSD cross-reference to the 3.51-rated assessment). The -0.5 TVL modifier is not applied (the protocol had a material Mar-2026 bad-debt incident, disqualifying a discretionary reduction).
...
**Final Risk Tier: LOW RISK** (moved from Minimal Risk; final score 1.4 → 2.4; driven by SUSDAI concentration above the 30% trigger, sUSDAI issuer identified as USD.AI, the USDai vault oracle found to be a depeg-blind hardcoded 1:1 peg at ~94% CF, ~37% of the collateral book not onchain-verifiable against true value, REUSD 3.51 cross-reference, and non-application of the -0.5 TVL modifier due to the Mar 2026 incident)
...
Factors driving the score movement from 1.4 to 2.4: (1) the **30% concentration trigger firing** — SUSDAI is now 31.2% of cross-chain TVL and still rising; (2) the **identification of sUSDAI's issuer as USD.AI**, which surfaced that Fluid holds ~two-thirds of all sUSDAI supply (~$199M) with extreme on-chain illiquidity and that sUSDAI is not a stable-value instrument; (3) the **USDai vault oracle**, verified onchain to be a **depeg-blind hardcoded 1:1 peg** (`PegOracleL2`) at ~94% collateral factor — a USDai depeg would leave positions real-underwater with liquidations never firing, a structural bad-debt path on the dominant collateral identical in shape to March 2026's wstUSR event; (4) **cross-referencing the existing REUSD risk assessment** (score 3.51/5.0, Medium Risk), which shows $38.8M (6.1% of Fluid TVL, ~26.7% of reUSD supply) is in an asset with ~50% offchain backing, admin-written share prices, EOA reserve custody, and no bug bounty; and (5) the finding that for **~37% of the collateral book the backing cannot be onchain-verified against true value** (reUSD via admin-written NAV; USDai via a hardcoded peg over an unverifiable offchain loan book). Combined SUSDAI + REUSD: ~37% of TVL in yield-bearing wrappers from two high-risk issuers, one of which we independently rate Medium Risk. **The -0.5 TVL modifier is not applied** — a protocol with a material bad-debt event in the past ~4 months should not receive a discretionary score reduction. Funds Mgmt § A (Collateralization) moves 2.75 → 4.0, § B (Provability) 1.0 → 2.0, and Category 2 § C (Dependencies) 2.5 → 3.5 (weighted subtotal 1.94 → 2.375). At one-decimal rounding the final score moves to 2.4, shifting from Minimal Risk to **Low Risk** tier — now sitting just under the Medium boundary, appropriate for a protocol whose dominant collateral is a concentrated, illiquid, peg-oracle-priced synthetic dollar larger than the exposure that produced its March 2026 bad-debt event.
...
- **Concentration-based (FIRED — Jul 6 2026)**: ~~Reassess immediately if SUSDAI exceeds 30% of cross-chain lending TVL~~ → **TRIGGER FIRED at 30.6% (Jul 6), now 31.2% (Jul 14)**. Reassess **immediately** if SUSDAI exceeds **35%** of cross-chain lending TVL (new threshold) OR if any per-chain single-asset concentration exceeds 85% (Plasma SUSDAI 75.3%, Arbitrum SUSDAI 63.9% — both still below).
...
- **USDai depeg (oracle-blind — highest priority)**: The USDai vaults use a **hardcoded 1:1 peg oracle** (`PegOracleL2`) that will *not* reflect a depeg and will *not* trigger liquidations. Off-chain monitoring of the USDai / sUSDai **market** price (DEX/CEX) is essential. Reassess **immediately** on any sustained USDai deviation below ~$0.97, or if the oracle for any USDai/sUSDai vault is changed. A depeg past ~1 − CF (≈6% at 94% CF) on maxed positions produces uncovered bad debt.
...
│   │  127 Executed              │      │  7-of-14 (unchanged Jul 14)  │  │
...
│      REUSD     6.1%  (Re Protocol — score 3.51 Medium Risk) ★                │
...
- **Collateral-issuer compromise → wrap-ratio amplified loss → bad debt**: the structural pattern that produced Mar 2026. **SUSDAI is 31.2% cross-chain and 64–75% on Arbitrum/Plasma** — larger than the wstUSR exposure that triggered the prior incident. **Issuer is USD.AI (usd.ai), a novel synthetic-dollar protocol backed by AI hardware loans.** Fluid holds ~two-thirds of all sUSDAI supply with near-zero exit liquidity (~0.2% daily volume). sUSDAI has traded as low as $0.796. Now also present on Ethereum (15.2%). **REUSD (6.1%, $38.8M)** has a standalone risk score of **3.51/5.0 (Medium Risk)** per [`reports/report/re-reusd.md`](re-reusd.md): ~50% offchain backing, admin-written share prices, EOA reserve custody, no bug bounty. Combined SUSDAI + REUSD = ~37% of TVL in high-risk yield-bearing wrappers.
...
| July 14, 2026 | 2.4 | Reassessment: lending TVL $639.0M (−26.8% off May peak, stabilizing); SUSDAI above 30% trigger at 31.2% and issuer identified as USD.AI (Fluid holds ~2/3 of supply, extreme illiquidity); USDai vault oracle verified as a depeg-blind hardcoded 1:1 peg (`PegOracleL2`) at ~94% CF; reUSD cross-referenced to its 3.51 Medium-Risk report; ~37% of collateral not onchain-verifiable against true value. Collateralization 2.75 → 4.0, Provability 1.0 → 2.0, Dependencies 2.5 → 3.5; GovernorBravo corrected to 127/135 executed. Minimal → Low Risk |
