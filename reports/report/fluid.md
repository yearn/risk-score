...
- **Final Score: 2.3/5.0** (up from 1.4; weighted subtotal 1.94 → 2.34 as SUSDAI crossed the 30% trigger, the issuer and Arbitrum source chain were identified as USD.AI, Ethereum sUSDai's bridge/mint path was verified, the existing REUSD risk assessment [score 3.51, Elevated Risk] was cross-referenced, and ~37% of the collateral book was found to depend on offchain/admin-side value inputs; -0.5 TVL modifier removed — protocol had a material bad-debt incident in Mar 2026)
...
Yield-bearing stablecoin wrappers are **~25% of Ethereum supply**, now the second-largest category behind ETH/LSTs and larger than plain stablecoins. SUSDAI is present on Ethereum at **$64.6M (15.2% of Eth supply)**, up from ~$52M on Jul 6 and near-zero in May — its expansion onto Ethereum adds to the cross-chain concentration concern. This is a **bridged representation whose source chain is Arbitrum**, not a native Ethereum ERC-4626 vault; the bridge/mint authority is documented below.
...
**sUSDai cross-chain architecture and Ethereum mint authority (verified Jul 22, 2026):** The canonical sUSDai contract is the Arbitrum [`StakedUSDai`](https://arbiscan.io/address/0x0b2b2b2076d95dda7817e785989fe353fe955ef9), an ERC-4626 vault whose `asset()` is Arbitrum USDai [`0x0A1a…82EF`](https://arbiscan.io/address/0x0A1a1A107E45b7Ced86833863f482BC5f4ed82EF). The same address on Ethereum, [`0x0b2b…955ef9`](https://etherscan.io/address/0x0b2b2b2076d95dda7817e785989fe353fe955ef9), is instead an upgradeable `OToken`: it has no `asset()` / `convertToAssets()` path and is minted or burned by `OAdapter` [`0xffB200…7f24`](https://etherscan.io/address/0xffB20098FD7B8E84762eea4609F299D101427f24), the sole current `BRIDGE_ADMIN_ROLE` holder. The adapter and token `DEFAULT_ADMIN_ROLE` are controlled by a [3-of-3 Safe](https://etherscan.io/address/0x5F0BC72FB5952b2f3F2E11404398eD507B25841F). Proxy upgrades are controlled by `ProxyAdmin` [`0x0b3296…d9F`](https://etherscan.io/address/0x0b3296b6f50611B28d466a6D5A49754dAd4D8d9F), owned by a [48-hour TimelockController](https://etherscan.io/address/0x0EEA1EE08611fF4A4E83BFe3916712751995639b). Fluid's Ethereum Liquidity Layer held ~67.5M of ~70.2M mainnet sUSDai supply when rechecked Jul 22 (~96%); at the Jul 14 report snapshot it accounted for the stated $64.6M. A bridge-adapter compromise, malicious role grant, or delayed/censored cross-chain settlement can therefore impair the Ethereum position independently of Fluid's vault oracles. The 3-of-3 Safe and 48-hour upgrade timelock mitigate unilateral and instant administrative changes, but they do not remove bridge dependency.
...
- External collateral-issuer dependency remains a first-order risk (Mar/Apr 2026 events). The SUSDAI concentration at 31.2% (cross-chain) and 64–75% (per-chain on Arbitrum/Plasma) makes a single novel issuer (**USD.AI**) the dominant external dependency. Fluid holds ~$199M of sUSDAI — roughly two-thirds of its ~$300M supply — with 24h on-chain volume of only ~$681K, making any exit from the position effectively impossible without cascading price impact. USD.AI has no standalone Yearn risk assessment and is newer and less-scrutinized than the issuers behind Fluid's blue-chip collateral. Ethereum sUSDai also adds a cross-chain `OAdapter` mint/burn dependency controlled by USD.AI's 3-of-3 Safe, with upgrades behind a separate 48-hour timelock. **Additionally, REUSD ($38.8M, 6.1% cross-chain) has a standalone risk score of 3.51/5.0 (Elevated Risk)** per [`reports/report/re-reusd.md`](re-reusd.md): ~50% offchain backing via reinsurance trusts, share price written daily by admin-controlled Chainlink Functions (`setSharePrice`), ~86% of onchain reserves held at plain EOAs, no bug bounty, and only 42% branch test coverage. Fluid holds ~26.7% of all reUSD supply.

**Score: 2.33/5** (was 2.0 in May). (2.0 + 1.5 + 3.5) / 3 = 2.33. Dependencies subcategory raised 2.5 → 3.5: (a) sUSDAI issuer identified as USD.AI — a single novel synthetic-dollar issuer on which 31.2% of lending TVL depends, where Fluid holds ~two-thirds of total supply, the Ethereum position depends on a privileged cross-chain mint/burn adapter, and no standalone assessment exists (2.5 → 3.0); plus (b) **REUSD cross-reference** to the existing 3.51-rated (Elevated Risk) assessment at `reports/report/re-reusd.md`, an asset with ~50% offchain backing, admin-written share prices, EOA-based reserve custody, and no bug bounty (3.0 → 3.5). With sUSDAI (31.2%) and REUSD (6.1%), ~37% of Fluid's lending TVL is in yield-bearing wrappers from two high-risk issuers (USD.AI and Re Protocol), one of which we independently rate Elevated Risk.

#### Category 3: Funds Management (Weight: 30%)

**Subcategory A: Collateralization — 3.75** (was 3.25 on Jul 6, 3.00 mid-reassessment, 2.75 in May, 2.5 in Apr)
...
- **Ethereum bridge dependency:** The $64.6M Ethereum sUSDai position is a bridged `OToken`, not collateral in a mainnet USDai/sUSDai T1 vault. Its value ultimately depends on canonical Arbitrum sUSDai plus correct operation of the privileged `OAdapter` mint/burn path described above.
- **Calibration:** 3.75 sits above Maple syrupUSDC (3.0, more diversified/transparent collateral) and approaches — but stays below — reUSD (4.25) and structurally-undercollateralized credit books (3jane/infinifi 4.5), because Fluid's per-position over-collateralization and functioning onchain liquidations are real. The realized Mar-2026 loss at a *smaller* concentration, a #1 asset worse than reUSD, extreme exit illiquidity, and Ethereum bridge dependency lift it from the "mixed quality" 3.0–3.25 band to 3.75. The currently tiny USDai T1 hard-peg debt is tracked as a specific risk rather than attributed to the whole sUSDai book.
...
- **However, ~37% of the collateral book depends on offchain/admin-side value inputs that cannot be independently verified onchain**, so whether the fTokens are actually fully backed is *not* purely onchain-provable:
  ...
  - **sUSDAI** ($199.2M): its value derives from USD.AI's offchain AI-hardware loan book; there is no onchain proof of the underlying collateral, and its market price is set on a venue Fluid itself dominates. On Ethereum, Fluid additionally relies on the privileged sUSDai `OAdapter` to keep the bridged `OToken` supply synchronized with canonical Arbitrum sUSDai.
...
**Score: 2.875/5** — (3.75 + 2.0) / 2 = 2.875 (was 1.875 on Jul 6). Collateralization 3.25 → 3.75 for the collateral-quality, illiquidity, realized-loss, and bridge-dependency factors above; Provability 1.0 → 2.0 because ~37% of the collateral depends on offchain/admin-side value inputs (reUSD `setSharePrice`; USD.AI loan-book NAV), with Ethereum sUSDai adding a privileged cross-chain mint/burn path.
...
| Funds Management | 2.875 | 30% | 0.8625 |
| Liquidity Risk | 2.0  | 15% | 0.300 |
| Operational Risk | 1.5  | 5% | 0.075 |
| **Subtotal** | | | **2.3375** |

**Final Score: 2.3375** → rounded to **2.3** (was 1.4 in May 2026; was 1.1 in Feb 2026). Underlying weighted subtotal moved 1.94 → 2.34 driven by: (a) SUSDAI concentration crossing and holding above the 30% trigger, sUSDAI issuer and Arbitrum source chain identified as USD.AI, extreme exit illiquidity, and the Ethereum `OAdapter` bridge/mint dependency — Funds Mgmt § A (Collateralization) 2.75 → 3.75; (b) Funds Mgmt § B (Provability) 1.0 → 2.0 (~37% of the collateral book depends on offchain/admin-side value inputs — reUSD `setSharePrice`, USD.AI loan-book NAV — while Ethereum sUSDai adds a privileged cross-chain mint/burn path); (c) Category 2 § C (Dependencies) 2.5 → 3.5 (single novel issuer USD.AI + reUSD cross-reference to the 3.51-rated Elevated Risk assessment). The USDai T1 hard-peg oracle remains a specific structural issue, but its currently tiny vault debt is not attributed to the $199.2M sUSDai supply position. The -0.5 TVL modifier is not applied (the protocol had a material Mar-2026 bad-debt incident, disqualifying a discretionary reduction).
...
**Final Risk Tier: LOW RISK** (moved from Minimal Risk; final score 1.4 → 2.3; driven by SUSDAI concentration above the 30% trigger, sUSDAI issuer and Arbitrum source chain identified as USD.AI, Ethereum bridge/mint dependency, ~37% of the collateral book relying on offchain/admin-side value inputs, reUSD 3.51 Elevated Risk cross-reference, and non-application of the -0.5 TVL modifier due to the Mar 2026 incident)
...
Factors driving the score movement from 1.4 to 2.3: (1) the **30% concentration trigger firing** — SUSDAI is now 31.2% of cross-chain TVL and still rising; (2) the **identification of sUSDAI's issuer and source-chain architecture** — canonical sUSDai is an Arbitrum ERC-4626 vault, while Ethereum uses an upgradeable `OToken` whose mint/burn adapter and role admin are controlled by a 3-of-3 Safe (upgrades are behind a 48-hour timelock); (3) Fluid holds ~two-thirds of all sUSDAI supply (~$199M) with extreme on-chain illiquidity, and sUSDAI is not a stable-value instrument; (4) **cross-referencing the existing reUSD risk assessment** (score 3.51/5.0, Elevated Risk), which shows $38.8M (6.1% of Fluid TVL, ~26.7% of reUSD supply) is in an asset with ~50% offchain backing, admin-written share prices, EOA reserve custody, and no bug bounty; and (5) the finding that **~37% of the collateral book depends on offchain/admin-side value inputs** (reUSD `setSharePrice`, USD.AI loan-book NAV). Combined SUSDAI + REUSD: ~37% of TVL in yield-bearing wrappers from two high-risk issuers, one independently rated Elevated Risk. The separate USDai T1 hard-peg oracle can miss a depeg at 94% CF, but the affected Arbitrum T1 debt was only ~$365 at the Jul 22 recheck and is not the $199.2M sUSDai position. **The -0.5 TVL modifier is not applied** — a protocol with a material bad-debt event in the past ~4 months should not receive a discretionary score reduction. Funds Mgmt § A (Collateralization) moves 2.75 → 3.75, § B (Provability) 1.0 → 2.0, and Category 2 § C (Dependencies) 2.5 → 3.5 (weighted subtotal 1.94 → 2.34). At one-decimal rounding the final score moves to 2.3, shifting from Minimal Risk to **Low Risk** tier.
...
- **sUSDai source-chain / bridge controls (highest priority)**: Reassess immediately on any `BRIDGE_ADMIN_ROLE` or `DEFAULT_ADMIN_ROLE` change on Ethereum sUSDai, `OAdapter` owner change, peer/bridge configuration change, proxy implementation upgrade, cross-chain supply mismatch, or loss of correspondence with canonical Arbitrum sUSDai.
...
| July 14, 2026 | 2.3 | Reassessment: lending TVL $639.0M (−26.8% off May peak, stabilizing); SUSDAI above 30% trigger at 31.2%; issuer and Arbitrum source chain identified as USD.AI (Fluid holds ~2/3 of supply, extreme illiquidity); Ethereum `OAdapter` bridge/mint path verified; separate USDai T1 hard-peg oracle finding scoped to its own currently tiny debt; reUSD cross-referenced to its 3.51 Elevated-Risk report; ~37% of collateral depends on offchain/admin-side value inputs. Collateralization 2.75 → 3.75, Provability 1.0 → 2.0, Dependencies 2.5 → 3.5; GovernorBravo corrected to 127/135 executed. Minimal → Low Risk |
