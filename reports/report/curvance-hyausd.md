...
> **The exit mechanism has been tested.** Between August 17 and September 7, 2026 the vault processed 140+ `Withdraw` events totalling ~17.0M AUSD — a **27%** NAV drawdown that was **redemption, not loss**: the exchange rate *rose* from 1.007984 to 1.011141, and the exits were serviced by a vault that never held more than ~3.9M of instant liquidity.
...
| Trail of Bits | **December 9, 2025** (filename says 2025.5.23; the PDF cover and fix-review are dated Dec 9, 2025) | Curvance security assessment; Appendix D fix review states **all 17 issues resolved** | [PDF](https://github.com/curvance/curvance-contracts/blob/develop/contracts/audits/Trail%20of%20Bits%201%20-%202025.5.23.pdf) |
...
**Remediation status is published, and it is good.** Each report carries its own fix review: Trail of Bits states verbatim *"Curvance has resolved all 17 issues disclosed in this report"* (Detailed Fix Review Results); Sherlock's remediation overview shows no finding left both unfixed and unacknowledged (33 "resolved", 19 "fixed", 6 "acknowledged" across the text); TrustSec 1 records 123 "fixed" and 37 "acknowledged"; TrustSec 2 records 8 "fixed" and 4 "acknowledged". No finding was located that remains open and unacknowledged.
...
- **The assessed vault is out of bounty scope.** The program states: *"Only Curvance contracts with a populated Monad mainnet address on the Monad Contract Addresses page and currently in active use are in scope. Blank or placeholder entries… are out of scope."* On that page, the Earn Vaults section contains a single row — "AUSD Flagship" — with an **empty address cell**. [`0xaD663aC8…00Bf`](https://monadscan.com/address/0xaD663aC84052b52BE4ed1b27BA416505e84a00Bf) does not appear anywhere on the page. Coverage of the two markets underneath it is **split**: the savUSD market [`0xD1BFEA17…91D4`](https://monadscan.com/address/0xD1BFEA1728ffe98F515f26082fACfcc3341691D4) and its manager [`0x4B0a39eC…09F7`](https://monadscan.com/address/0x4B0a39eCC3e5A5dA3ce6D492D4D255cD1F0209F7) **are** listed, under "Avant Markets", so they are in scope; the PT market [`0xbDe2459A…6c93`](https://monadscan.com/address/0xbDe2459A033f20442CE18483CDD48643D80C6c93) and its manager [`0xdDd5681f…ec2e`](https://monadscan.com/address/0xdDd5681fCBB040e3bD3c126AA6caf1677DFCEc2e) are not. The uncovered surface is therefore the vault itself plus the 60% PT leg, not the whole stack. Verified by string search of the page markdown on September 8, 2026.
...
> ### 🔴 CRITICAL ISSUE — savUSD collateral (40% of the vault)
>
...
>
> Compounding it, the Curvance oracle prices savUSD as `USDC/USD × savUSD/avUSD`
> with **no avUSD/USD feed anywhere in the route** — so avUSD is *assumed* to be
> worth $1. If avUSD breaks, the 92%-LTV book does not liquidate, and the resulting
> bad debt is socialised directly into hyAUSD's exchange rate. **Opaque backing and
> an oracle that cannot price it is the combination that makes this critical rather
> than merely custodial.**
>
> Every admin power over avUSD, savUSD, the minting contract and the Monad CCIP pool
> sits on **one address with no onchain quorum and no timelock**, able to appoint
> itself an avUSD minter via `setMinter`.
>
> **This does not trigger the "Unverifiable reserves" critical gate**, and the
> distinction matters: hyAUSD's *own* reserves are fully onchain and reconcile
> exactly (see below). What is unverifiable is the collateral two layers down, which
...
>
> **Full inventory: [Appendix B — savUSD Risk Inventory](#appendix-b-savusd-risk-inventory).**
...
- **Security posture is more developed than the key structure suggests.** Avant publishes **eight** smart-contract audit files and states no critical or high-severity findings, runs a continuous Trail of Bits OPSEC engagement, and uses Hypernative for real-time threat monitoring. Avant's [Security Partners](https://docs.avantprotocol.com/security/security-partners) page names the firms: **Omniscia** and **Dedaub** on the smart contracts, **Trail of Bits** on OPSEC. The PDFs themselves are served as GitBook file blobs, so per-report dates and findings counts could not be extracted from the page text.
...
| [`0xc3a29745…21a1`](https://monadscan.com/address/0xC3a2974593BB729e5A62fE74a8ab091Fed5321A1), [`0x7d89822c…4441`](https://monadscan.com/address/0x7D89822C41191541D6A02AebC23f4F398D6E4441), [`0x4dad55e3…a012`](https://monadscan.com/address/0x4DaD55E314CF2FbB8e65B2AAC7d6010cc1b6A012), [`0xb68f3241…c3a4`](https://monadscan.com/address/0xB68F32416471a6f7a5753BA9e5baE5710219C3A4), [`0x1cfeaa99…38cc`](https://monadscan.com/address/0x1Cfeaa998b271228c6f4E090B8E5A007a08C38Cc) | — | — | ✓ | — | Five `ProtocolManager` instances, each with one immutable bot caller and per-parameter limits. **None manages this vault's markets.** |
...
**Signer sets** (thresholds and counts only — per `reports/TEMPLATE.md`, onchain multisig signer identities are not validated in these assessments): DAO Safe **3-of-4**, Emergency Council **4-of-5**, mass-pause Safe **1-of-5**. The material fact is the overlap: the 1-of-5 pause Safe is composed of the **same five signers as the 4-of-5 Emergency Council**, so the freeze key is not an independent party — it is the Emergency Council operating under a lowered threshold.
...
| **Avant avUSD/savUSD** | Collateral in market B | **Critical** | ~99.4% of avUSD backing sits outside Avant's contracts, partially traceable via 23 published strategy addresses but unattested; the sole registered custodian is an EOA; one EOA holds every admin key across avUSD, savUSD, the minting contract and the CCIP pool, and can mint unbacked avUSD via `setMinter`. A strategy loss or a key compromise impairs market B, and the Curvance oracle has no avUSD/USD feed to see it |
...
- **Documentation: good in the places that matter, stale in one that matters a lot.** The Earn Vaults page and the in-repo `docs/lending-optimizer-integration.md` are unusually candid — they self-disclose the loss-recognition race, the non-continuous caps, the fee-dust forgiveness and the transitive-dependency gap. That is above-average transparency. **But the published [Monad Contract Addresses](https://docs.curvance.com/app/protocol-overview/monad-contract-addresses) page is materially out of date**: the Earn Vaults section lists one row ("AUSD Flagship") with a blank address; neither hyAUSD nor either of its markets or market managers appears; and the `OracleManager` it lists ([`0x32faD39e…CDb6`](https://monadscan.com/address/0x32faD39e79FAc67f80d1C86CbD1598043e52CDb6)) is **not** the one `CentralRegistry` actually points at ([`0x65ADF8aE…37c5`](https://monadscan.com/address/0x65ADF8aE8420A58278De066593E6fF1713A137c5)). Since the bug-bounty scope is defined by that page, the staleness has a direct security consequence, not just a documentation one.
...
**hyAUSD-as-collateral markets** (reflexive leverage — these compete for the same exit liquidity, see Liquidity Risk)

| What | How | Threshold | Frequency |
|---|---|---|---|
| hyAUSD locked as collateral | `hyAUSD.balanceOf()` on [`0xbaAD847D…cA30`](https://monadscan.com/address/0xbaAD847D1Adcf143eE4Eb4DCFDd17dbB93d9cA30) and [`0xc3DfC3CD…7F28`](https://monadscan.com/address/0xc3DfC3CD69C97877720f5386f8d4742B0F147F28); `marketCollateralPosted()` on each | Combined holdings **>5% of hyAUSD supply** (2.72% today), or a >50% increase in 7 days — a forced unwind at that size would consume most of the ~$3.9M idle-cash budget | Daily |
| Their health / pause state | `redeemPaused()`, `liquidationPaused()`, `actionsPaused(cToken)` on managers [`0x120E3ef1…cE6a`](https://monadscan.com/address/0x120E3ef143457F9F4d1E3eCDC448a5bb1092cE6a) and [`0x8AF3E980…5974`](https://monadscan.com/address/0x8AF3E980d2d50F8E97dcD7DBbD7cAa0d17605974) | Any value != 1, or utilisation >90% — a liquidation cascade here forces hyAUSD redemptions | Hourly |

**Governance — [`CentralRegistry 0x1310f352…12fF`](https://monadscan.com/address/0x1310f352f1389969Ece6741671c4B919523912fF)**

| What | How | Threshold | Frequency |
|---|---|---|---|
| Permission grants | Events `PermissionsUpdated(string,address,bool)` (topic0 `0x40f7b3ef64c9e38288873dda008188d6c3cd146f84d77b978478a8731f71feb9`) and `PermissionsTransferred(string,address,address)` (topic0 `0xd8e127776f279a25ebbafc660f7deba16e71dcb8ff27626f743010802c27211d`) | **Any grant of `Market`, `Harvest` or a DAO/EC/Timelock transfer — page immediately** | Every block |
...
| Safe composition | `getOwners()` / `getThreshold()` on [`0x0Acb7eF4…5C02`](https://monadscan.com/address/0x0Acb7eF4D8733C719d60e0992B489b629bc55C02), [`0x379D4a8F…399c`](https://monadscan.com/address/0x379D4a8FBc23A8Fd8c2b3738Dbf1fEBe9a64399c), and the 1-of-5 pause Safe [`0x25D41348…0902`](https://monadscan.com/address/0x25D4134861b30Ba7215d2A280DD84C57c5780902) | Any signer or threshold change | Daily |
...
| Mass-pause keys | `ProtocolManagerMassPause` events `MassPauseExecuted`, `MarketPauseFailed` on [`0xbb82aee1…ad73`](https://monadscan.com/address/0xBB82Aee16936fB2232F3c0d700b6fc832D7Ead73) and [`0x89d2253b…7803`](https://monadscan.com/address/0x89D2253b13c718d06B78fCcE3d973a520bD07803) | Any execution | Every block |
...
## Appendix B: savUSD Risk Inventory

Everything found wrong with the savUSD collateral leg, in one place. Every row was
verified onchain on Avalanche (chain 43114) and Monad (chain 143) on **August 17, 2026**,
or read from Avant's published source. This is the evidence behind
[Critical Risk #1](#critical-risks) and the callout in [Funds Management](#funds-management).

**Why this leg matters:** it is **40% of hyAUSD's assets** (7,332,183 AUSD), lent at
**92% LTV** into a market at 89.66% utilisation, where bad debt socialises directly into
the cAUSD exchange rate and therefore into hyAUSD's NAV. The allocation cap was raised
**20% → 30% (July 4, 2026) → 40% (August 13, 2026)**, both times through the Emergency
Council with no timelock, and the vault currently sits **at** that cap.

### B.1 The backing is almost entirely unverifiable
...
`AvantMintingV2._transferCollateral()` runs `token.safeTransferFrom(benefactor, addresses[i], …)`,
so depositor USDC/USDT goes **straight to custodian addresses at mint time and never enters a
protocol contract**. That is why the escrow-then-sweep path has never once been used.

**Where the rest actually is — checked, not assumed.** Avant publishes **23 avUSD strategy addresses**
([Strategy Addresses](https://docs.avantprotocol.com/yield-strategies-and-revenue/strategy-addresses)).
All were queried on August 19, 2026: the 20 EVM addresses hold **$1,194,937** in USDC+USDT combined, of
which **$1,194,287 is the custodian already counted above** — so enumerating them adds ~$650 and does
not move the figure. The other three are a **Solana** wallet (~11.5K of token balances), a **Stacks L1**
wallet, and a **Lighter** (perp DEX) address. All 20 EVM addresses are EOAs, 16 with transaction
history, holding almost no idle stablecoins — the shape you would expect when collateral is deployed
into delta-neutral positions on exchange venues.

So the accurate statement is **partially traceable, not attested**: Avant discloses *where* the wallets
are, but there is no consolidated proof that the sum matches liabilities. Avant's
[Security Partners](https://docs.avantprotocol.com/security/security-partners) page states that a
Chainlink **Proof of Reserve** is *"actively developing"* and *"planned"* — i.e. **not live** — and the
OPSEC audit findings are explicitly not public. This remains the sharpest contrast with Ethena, the
protocol Avant is forked from essentially line-for-line, which publishes per-custodian reserve
breakdowns today.

### B.2 The oracle cannot see a depeg

Curvance prices savUSD through `CombinedAggregator` [`0x7CB9a321…4999`](https://monadscan.com/address/0x7CB9a321c30c753c3F7C6af7Ae8776E5C1524999) as:

```
savUSD/USD  =  USDC/USD  ×  (savUSD/avUSD exchange rate)
```

**There is no avUSD/USD feed anywhere in the route** — avUSD is assumed to be worth exactly one
USDC. The `savUSD/avUSD` leg is a genuine Chainlink OCR2 feed and the rate it reports is sound, but
it only measures staking accrual, not solvency. Curvance's own contract comments name this exact
limitation: *"if AUSD depegs and loses value, this may remain hidden from the guard due to an
increase in the [secondary] rate… the primary price [is left] without an effective guard."*

Consequence: **an avUSD depeg does not move the collateral price, so 92%-LTV loans do not become
liquidatable.** The book silently goes underwater and the loss reaches hyAUSD only when the cToken
recognises it. All three assets also resolve through a **single** `ChainlinkAdaptor`
...
| Contract | Chain | Powers held |
|---|---|---|
| avUSD [`0x24dE8771…E346`](https://snowtrace.io/address/0x24dE8771bC5DdB3362Db529Fc3358F2df3A0E346) | Avalanche | `owner()` → `setMinter(address,bool)` is `onlyOwner` |
| savUSD [`0x06d47F3f…219E`](https://snowtrace.io/address/0x06d47F3fb376649c3A9Dafe069B3D6E35572219E) | Avalanche | `owner()` + `DEFAULT_ADMIN_ROLE` |
| `AvantMintingV2` [`0xcb43139E…A49c`](https://snowtrace.io/address/0xcb43139E90f019624e3B76C56FB05394B162A49c) | Avalanche | `owner()` + `DEFAULT_ADMIN_ROLE` + `COLLATERAL_MANAGER_ROLE` |
| savUSD (bridged) [`0x9648dB94…06c6`](https://monadscan.com/address/0x9648dB94F1e6B19e7D755585542981F97dc806c6) | Monad | `owner()` + `getCCIPAdmin()` |
| CCIP pool [`0xc5cAAC64…8D3E`](https://monadscan.com/address/0xc5cAAC64Dd93b6E2B369d52610Fdae069e568D3E) | Monad | `owner()`; `getRateLimitAdmin()` is unset, so limits are owner-only |

`avUSD` is `Ownable2Step` and `setMinter` is `onlyOwner`, so **this key can appoint itself a minter
and issue unbacked avUSD, with no cap of any kind on that path**. The 25,000,000 `maxMintPerBlock` limit does **not** apply here: it is enforced by the `belowMaxMintPerBlock` modifier on `AvantMintingV2.mint()` only. `AvUSD.mint()` is simply `if (!minters[msg.sender]) revert OnlyMinter(); _mint(to, amount);` — a minter added directly through `setMinter` is unbounded, and the admin also sets `maxMintPerBlock` in any case.

**Stated precisely, because the distinction is easy to get wrong:** an MPC wallet is
indistinguishable from an EOA onchain, and Avant documents MPC custody for its Reserve Fund wallets
plus a continuous Trail of Bits OPSEC engagement covering treasury and access controls. So "no code"
does **not** establish a hot key on a laptop. What *is* established: **no onchain quorum and no
timelock** on any of the powers above. The offchain signing policy is undisclosed and should be
treated as unverified.

### B.4 Blacklist-and-seize retained from Ethena

`StakedAvUSD` keeps `SOFT_RESTRICTED_STAKER_ROLE`, `FULL_RESTRICTED_STAKER_ROLE` and
`redistributeLockedAmount(from, to)` — `DEFAULT_ADMIN_ROLE` can blacklist a holder, burn their
entire savUSD balance and reassign it. This acts on the **Avalanche** token, so it does not reach
the bridged Monad collateral directly, but it applies to anyone bridging back and to the canonical
pool the Monad token redeems against.

### B.5 The bridge can mint the collateral
...
### B.6 The first-loss buffer is real but thin, and partly circular
...
> **Note on this figure.** avUSDx and savUSD have different unit values, so their balances are
> converted to avUSD before being added together.
...
Stated for balance, because these are real and were checked:

- **Eight** published smart-contract audit files, with a claim of no critical or high-severity
  findings. The PDFs are GitBook file blobs so per-report dates could not be extracted, but Avant's
  [Security Partners](https://docs.avantprotocol.com/security/security-partners) page names the
  firms: **Omniscia** and **Dedaub** for the smart contracts, **Trail of Bits** for ongoing OPSEC.
- A continuous **Trail of Bits OPSEC** engagement (internal policy, treasury controls, infrastructure
  access, incident response); findings deliberately not public.
- **Hypernative** real-time threat monitoring.
- A senior/junior tranche design where savUSD is explicitly the senior, most-protected layer.
- $6.5M seed funding and an established Avalanche-ecosystem presence.
...
- **The staking vault reconciles.** `totalAssets()` 108,210,389 against an actual avUSD balance of
  108,231,057.

### B.9 What would resolve this

1. **Onchain multisig + timelock** on the Avant admin key — removes the single-key mint and role powers.
2. **Published reserve attestation** with named custodians and per-custodian balances, at a committed
   cadence — the Ethena standard Avant is currently below.
3. **An avUSD/USD oracle feed** in the Curvance route — this one is Curvance's to fix, not Avant's, and
   is the single highest-leverage change, because it converts a silent solvency failure into a normal
   liquidation event.
4. **A hard, agreed ceiling on the market-B allocation cap**, given it has moved 20% → 30% → 40% in six
   weeks with no timelock and is currently at cap.
...
- **Audited stack underneath, with real emergency tooling.** The isolated-market layer is covered by Sherlock and TrustSec ×2, the Trail of Bits invariant-development engagement, and Cantina's public competition; purpose-built mass-pause contracts and a defensive-routing planner exist and are deployed.
...
4. **Single-signer withdrawal freeze**, recoverable only by a different key. Any position must be sized assuming exits can be blocked without warning.
5. **CCIP mint path on the Monad collateral.** Independently of Avant's custody, a compromise of the CCIP path or the pool owner can mint unbacked savUSD directly on Monad — the Avalanche lane's inbound capacity is 44.88M savUSD against 15.36M actually posted — post it as collateral and borrow out market B's AUSD.
6. **First-mover exit advantage on unrecognised impairment.** Curvance documents that pooled liquidity can fund an exit before an impaired cToken recognises its loss, concentrating the loss on whoever is still in. A large, procedurally-slow holder such as a Yearn vault is structurally the last one out.
...
- [ ] **No audit** — **PASSES.** The gate is written at protocol level, and the Curvance lending stack the vault sits on has been reviewed by Sherlock (Sept 2025), TrustSec (Oct + Nov 2025), Trail of Bits (May 2025 + invariant development) and Cantina (Apr 2025). The gate is therefore not triggered. **But the assessed contract itself has no audit coverage**, so it is carried in Category 1 (1A scored 2 rather than 1) and as Critical Risk #1 rather than as a gate.
...
Six audits exist and five of them touch the lending layer the vault deposits into: Sherlock and TrustSec ×2 directly cover `BorrowableCToken`/`MarketManagerIsolated`, the Trail of Bits invariant-development engagement built the `MarketManager` invariant suite, and Cantina's public competition exercises the market contracts in its test material. By firm quality this is the rubric's "3+ audits by top firms" band (1). The bounty pays **Critical $250,000 / High $50,000 / Medium $5,000**, which is the rubric's "max payout >$200K" band (2). Docking from 1 to **2** reflects the two verified caveats that the audit-count bands do not capture: the 1,513-line `LendingOptimizer` custody contract itself appears **zero times** in all six PDFs, and it is **excluded from the bug bounty** because it is missing from the scope-defining docs page. The self-run email intake is noted but does not change the payout band. Remediation discipline is a genuine positive — Trail of Bits records all 17 issues resolved, and Sherlock and both TrustSec reports carry fixed/acknowledged statuses with nothing left open and unacknowledged — but it does not extend coverage to the contract being assessed. The contract surface is moderately complex at the vault level and substantially complex underneath. **2**.

**Subcategory B: Historical Track Record — 4/5**

Vault deployed **June 23, 2026 — 76 days ago**, still "<3 months" (5). Scale is $13.39M, which is "≥$10M" (3). The protocol beneath is 9 months live on Monad with $130.09M supplied TVL (September 6, 2026) and one non-contract incident handled cleanly, which pulls the blend up; holder concentration (top 5 = 78.32%) and the fact that the two markets are themselves only ~2 months old push it back down. **4**.

**Audits & Historical Score = (2 + 4) / 2 = 3.0**

**Score: 3.0/5** — heavily audited stack, an unaudited and out-of-bounty vault, two months of history.
...
**Why 4 and not 5 (or 4.5).** The band above is defined as *"Single point of failure dependency / Failure breaks entire protocol"* — a structurally different claim from "many critical dependencies," which is the 4 band and describes this vault exactly. No single dependency here breaks the protocol on its own: the Avant leg is capped at 40% and sits behind a ~14% first-loss buffer; the CCIP mint path is bounded by the collateral cap and cannot seize; Pendle PT redeems at par in weeks; the oracle feeds are Chainlink OCR2 with fresh heartbeats. AUSD is the closest thing to a true single point of failure, but it is the vault's *denomination* rather than an external dependency — an AUSD vault failing when AUSD fails is definitional, not a dependency defect. Seven critical-but-independent dependencies is the 4 band; concentrating the score upward would double-count the Avant leg, which is already priced in Category 3 as Critical Risk #1.

**Centralization Score = (4 + 2 + 4) / 3 = 3.333**

**Score: 3.333/5** — immutable code, but zero-delay multisig control, a single-signer freeze key, and a long young-dependency chain.
...
**Score: 2.75/5** — impeccable accounting over collateral that is thinner, worse-priced and far less verifiable than the accounting implies, partly offset by a real ~14% first-loss buffer beneath the weaker leg.
...
| Centralization & Control | 3.333 | 30% | 1.000 |
...
- **Treat Critical Risk #1 as a precondition.** An audit of `LendingOptimizer` and the addition of [`0xaD663aC8…00Bf`](https://monadscan.com/address/0xaD663aC84052b52BE4ed1b27BA416505e84a00Bf) plus the PT market to the Monad Contract Addresses page (which puts them in the $250K bounty scope) would materially move Category 1 and the final score.
...
- **Time-based**: Reassess in **3 months** (by December 7, 2026), or sooner — the vault is 76 days old and the score is dominated by facts that change quickly. Over the past three weeks, NAV moved 27%, exit liquidity moved by a third, and top-holder concentration moved by 28 points — the natural cadence here.
