...
- Vault holds $0 USDC directly — 100% of ~$10.73M is deployed externally via MultisigStrategy. Importantly, this strategy is a **custody handoff** design: the vault calls `allocateFunds()`, the strategy's `_allocateToPosition()` forwards USDC directly to the Treasury multisig via `safeTransfer(multiSig, amount)`, and assets return only if the multisig cooperates with `_retrieveAssetsFromMultisig()` / `safeTransferFrom(multiSig, ...)`. There is no on-chain requirement that forwarded funds be deployed to approved markets rather than simply remaining under multisig custody.
...
- **Treasury custody is not escrow-enforced:** `onlyVault` restricts who can call strategy entrypoints, but does not constrain what happens after allocation. Once the vault allocates, `_allocateToPosition()` transfers the full amount to the Treasury multisig. The strategy cannot force deployment into Aave / Dawn markets, and `_retrieveAssetsFromMultisig()` requires the multisig to approve funds back. If the Treasury multisig colludes or simply refuses to cooperate, assets can be withheld from the vault without violating any strategy invariant.
...
- Funds are on-chain in Aave/Avant/Neutrl/Auto only if the Treasury multisig actually deploys them there. The strategy itself does not enforce deployment: `_allocateToPosition()` forwards custody to the multisig, and `_retrieveAssetsFromMultisig()` depends on multisig approval to pull assets back. The entire PPS therefore depends on both multisig accounting honesty and multisig custody cooperation (constrained on reporting by max 0.5% per update, 12-hour cooldown, nonce 37 confirming regular updates).
